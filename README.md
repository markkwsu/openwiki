...
- **Coding-agent integrations** for Codex, Claude Code, OpenCode, Cursor, and IBM Bob, using the host's model and repository tools.
...
- **OpenWiki integrations:** run OpenWiki directly inside Codex, Claude Code, OpenCode, Cursor, or IBM Bob, using the coding agent's authenticated model and native repository tools while OpenWiki manages the documentation lifecycle.
...
openwiki integrations install <codex|claude|opencode|cursor|bob> [--project [path]]
openwiki integrations uninstall <codex|claude|opencode|cursor|bob> [--project [path]]
