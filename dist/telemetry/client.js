...
import { DEFAULT_POSTHOG_HOST, DEFAULT_POSTHOG_KEY, FLUSH_TIMEOUT_MS, } from "./config.js";
/**
...
export async function capture(event) {
    ...
    let sent = false;
    try {
        sent = await withTimeout(Promise.resolve().then(() => client.captureImmediate({
        ...
        try {
            await withTimeout(Promise.resolve().then(() => client.shutdown()), FLUSH_TIMEOUT_MS);
        }
        catch {
