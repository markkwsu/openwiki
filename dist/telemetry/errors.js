import { deriveOwner, isSafeErrorIdentifier, normalizeErrorDetail, } from "./taxonomy.js";
/**
 ...
 */
export function unwrapErrorChain(error) {
    const chain = [];
 ...
 * the tag over this.
 */
function classifyErrorLink(error) {
    ...
    if (/context length|maximum context|too many tokens|context window/u.test(message)) {
 ...
 */
export function firstStatusInChain(error) {
    for (const link of unwrapErrorChain(error)) {
 ...
 */
function tagErrorOrigin(error, tag) {
    if (typeof error !== "object" || error === null) {
        return;
    }
    const target = error;
 ...
 */
function readOwnOriginTag(error) {
    if (typeof error === "object" && error !== null) {
        const tag = error[ERROR_ORIGIN];
 ...
 */
function readErrorOrigin(error) {
    let nearestStageOnly;
    for (const link of unwrapErrorChain(error)) {
 ...
 */
export function tagErrorStage(error, stage) {
 ...
 *   classification.
 */
export async function inStage(stage, fn, origin) {
    try {
        return await fn();
    }
    catch (error) {
 ...
 */
export function inStageSync(stage, fn, origin) {
    try {
        return fn();
    }
    catch (error) {
 ...
 */
export function safeConstructorName(value) {
 ...
 */
export function innermostErrorName(error) {
    let deepest;
    for (const link of unwrapErrorChain(error)) {
 ...
 * message.
 */
export function describeErrorForTelemetry(error) {
    ...
    let errorClass;
    let rawDetail;
    if (origin?.errorClass !== undefined &&
    ...
    const detailInput = errorClass === "agent_error" ? innermostErrorName(error) : rawDetail;
 ...
 */
function streamOpenDisguisesProvider(origin, classified, httpStatus) {
    if (origin?.errorClass !== "build_error" ||
        origin.errorDetail !== "stream_open") {
        return false;
    }
    return (classified.errorClass === "provider_error" ||
