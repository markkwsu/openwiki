...
import { getMissingProviderEnvKey, getProviderApiKeyEnvKey, getProviderBaseUrlEnvKey, getProviderLocationEnvKey, getProviderProjectEnvKey, getProviderRegionEnvKey, getProviderSecretKeyEnvKey, getProviderLabel, getDefaultModelId, getProviderModelOptions, normalizeModelId, isValidModelId, providerRequiresApiKey, providerRequiresBaseUrl, providerRequiresRegion, providerRequiresSecretKey, providerUsesAwsSdkCredentials, providerUsesExternalCliAuth, providerUsesOAuth, resolveConfiguredProvider, resolveProviderRegion, normalizeProvider, OPENWIKI_MODEL_ID_ENV_KEY, OPENWIKI_PROVIDER_ENV_KEY, OPENWIKI_REASONING_EFFORT_ENV_KEY, SELECTABLE_OPENWIKI_PROVIDERS, } from "../../config/constants.js";
import { getReasoningCapability } from "../../config/reasoning.js";
import { readCodexTokensFromEnv, isChatGptTokenExpired, } from "../../agent/openai-chatgpt-oauth.js";
import { createEmptyOnboardingConfig, isOnboardingComplete, isOpenWikiOnboardingCompleteSync, isRepositoryCodeOnboardingCompleteSync, readRepositoryWikiInstructions, } from "../onboarding.js";
import { ONBOARDING_TEMPLATES, RUN_MODE_OPTIONS, LANGSMITH_REGION_OPTIONS, SOURCE_OPTIONS, CRON_FIELD_LABELS, } from "./constants.js";
...
export function needsAwsCredentialRepair(provider) {
    return (providerUsesAwsSdkCredentials(provider) &&
 ...
 */
export function needsCredentialStep(provider) {
...
export function credentialStep(provider) {
    if (providerUsesOAuth(provider)) {
        return "oauth-login";
    }
 ...
 */
export function getWizardManagedEnvKeys(provider) {
    return [
...
export function getReasoningEffortSelectionOptions(provider, modelId) {
...
export function getReasoningEffortSelectionIndex(provider, modelId, value) {
 ...
 */
export function orderedSetupSteps(provider, mode, allowModeSelection) {
    const steps = [];
    ...
    if (getProviderProjectEnvKey(provider) &&
        getProviderLocationEnvKey(provider)) {
 ...
 */
export function nextSetupStep(step, provider, mode, allowModeSelection) {
    if (step === null) {
        return null;
    }
...
export function needsRegionStep(provider) {
    if (!providerRequiresRegion(provider)) {
        return false;
    }
 ...
 * has been recorded.
 */
export function needsLangSmithStep(env = process.env) {
    return !env.LANGSMITH_API_KEY && env.LANGCHAIN_TRACING_V2 === undefined;
}
export function isRegionConfigured(provider) {
    return resolveProviderRegion(provider) !== undefined;
}
export function isCredentialConfigured(provider) {
 ...
 */
export function resolveStepStatus(id, activeStep, done, resting = "pending") {
    if (id === activeStep) {
        return "current";
    }
    if (done) {
...
export function getInitialStep(modelIdOverride, provider, onboardingConfig = createEmptyOnboardingConfig(), mode = "code", allowModeSelection = false, walkAll = false) {
    if (walkAll) {
...
export function getNextStepAfterApiKey(provider, modelIdOverride, onboardingConfig, mode, forceModelStep = false) {
    if (needsSecretKeyStep(provider)) {
        return "secret-key";
    }
    return getNextStepAfterSecretKey(provider, modelIdOverride, onboardingConfig, mode, forceModelStep);
}
export function getNextStepAfterSecretKey(provider, modelIdOverride, onboardingConfig, mode, forceModelStep = false) {
    if (needsGcpProjectStep(provider)) {
        return "gcp-project";
    }
    return getNextStepAfterGcpLocation(provider, modelIdOverride, onboardingConfig, mode, forceModelStep);
}
export function getNextStepAfterGcpLocation(provider, modelIdOverride, onboardingConfig = createEmptyOnboardingConfig(), mode = "code", forceModelStep = false) {
    if (needsBaseUrlStep(provider)) {
        return "base-url";
    }
    return getNextStepAfterBaseUrl(provider, modelIdOverride, onboardingConfig, mode, forceModelStep);
}
export function getNextStepAfterBaseUrl(provider, modelIdOverride, onboardingConfig, mode, forceModelStep = false) {
    if (needsRegionStep(provider)) {
        return "region";
    }
    return getNextStepAfterRegion(provider, modelIdOverride, onboardingConfig, mode, forceModelStep);
}
export function getNextStepAfterRegion(provider, modelIdOverride, onboardingConfig, mode, forceModelStep = false) {
    if (modelIdOverride === null &&
...
export function ensureRunModeConfig(config, mode) {
...
export async function hydrateRunModeConfig(config, mode, repoRoot) {
    if (mode !== "code") {
        return config;
    }
...
export function getRunModeSelectionIndex(mode) {
    const index = RUN_MODE_OPTIONS.findIndex((option) => option.id === mode);
    return index === -1 ? 0 : index;
}
export function getLangsmithRegionSelectionIndex(region) {
    const index = LANGSMITH_REGION_OPTIONS.findIndex((option) => option.id === region);
    return index === -1 ? 0 : index;
}
export function getLangsmithRegionLabel(region) {
    const option = LANGSMITH_REGION_OPTIONS.find((item) => item.id === region);
    return option ? `${option.name} (${option.host})` : region;
}
export function getRunModeName(mode) {
    return RUN_MODE_OPTIONS.find((option) => option.id === mode)?.name ?? mode;
}
export function getSourceOption(sourceId) {
    return (SOURCE_OPTIONS.find((source) => source.id === sourceId) ?? SOURCE_OPTIONS[0]);
}
export function getConfigModeId(config) {
    return config.modeId ?? config.templateId;
}
export function getConfigModeName(config) {
    return config.modeName ?? config.templateName;
}
export function isCodeMode(config) {
    return getConfigModeId(config) === "code";
}
export function hasValidConfiguredProvider() {
    return normalizeProvider(process.env[OPENWIKI_PROVIDER_ENV_KEY]) !== null;
}
export function getDefaultCodeRepoRootPath() {
...
export function needsEnvValue(secretInput) {
    return !process.env[secretInput.envKey];
}
export function addSourceInstanceConfig(config, sourceInstance) {
...
export function deriveLegacySources(sourceInstances) {
    const sources = {};
...
export function getSourceInstanceCount(config, sourceId) {
    return getSourceInstances(config, sourceId).length;
}
export function getSourceInstances(config, sourceId) {
    return config.sourceInstances.filter((sourceInstance) => sourceInstance.connectorId === sourceId);
}
export function getConnectedSourceCount(config, sourceOptions) {
...
export function createSourceInstanceId(sourceId, config) {
    const sourceCount = getSourceInstanceCount(config, sourceId) + 1;
    return `${sourceId}-${sourceCount}`;
}
export function createSourceInstanceName(source, description, config) {
...
export function isSourceStep(step) {
 ...
 */
export function getApiKeyFieldLabel(provider) {
    return provider === "bedrock"
        ? `${getProviderLabel(provider)} access key ID`
        : `${getProviderLabel(provider)} API key`;
}
export function getModelSetupDetail(modelIdOverride, provider) {
    if (modelIdOverride) {
...
export function getModelSelectionOptions(provider) {
    return [
...
export function shouldStartWithCustomModelInput(provider) {
    return getProviderModelOptions(provider).length === 0;
}
export function getSelectedModelId(provider, selectedIndex, input, isCustomInput) {
    if (!isCustomInput) {
...
export function getProviderArticle(provider) {
...
export function getTemplateGoal(templateId) {
    return (ONBOARDING_TEMPLATES.find((template) => template.id === templateId)
        ?.suggestedGoal ?? "");
}
export function getSourceMenuLabel(source, sourceInstanceCount) {
    return sourceInstanceCount > 0
        ? `Add another ${source.displayName}`
        : `Add ${source.displayName}`;
}
export function getTemplateSourceOptions(templateId) {
...
export function getSourceDescriptionPrompt(source) {
...
export function getFinalOptionLabel(option, mode) {
    if (mode !== "code") {
        return option;
    }
    return option === "Run ingestion now" ? "Run OpenWiki now" : "Open chat";
}
export function getSourceDescriptionOptionCount(source) {
    return source.examples.length + 1;
}
export function handleCronEditorInput({ currentFieldIndex, currentValue, fallbackExpression, inputValue, key, replaceCurrentField, setCurrentFieldIndex, setReplaceCurrentField, setValue, }) {
    if (key.leftArrow) {
...
export function getCronFields(expression, fallbackExpression) {
...
export function parseCronFieldPaste(inputValue) {
...
export function sanitizeInputChunk(value) {
    return value.replace(/[\r\n]/gu, "");
}
export function sanitizeCronInputChunk(value) {
    return value.replace(/[^A-Za-z0-9*,/?#LW.-]/gu, "");
}
export function sanitizeRepoId(value) {
    return value.replace(/[^A-Za-z0-9._-]/gu, "-").slice(0, 80) || "repo";
}
export function getDefaultLocalGitRepoPath() {
    return process.cwd();
}
export async function validateLocalDirectoryPath(value) {
...
export function getStaticSourceConfig(sourceId, query) {
