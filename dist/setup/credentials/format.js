...
import { getMissingProviderEnvKey, getProviderApiKeyEnvKey, getProviderSecretKeyEnvKey, providerUsesAwsSdkCredentials, providerUsesOAuth, AWS_ACCESS_KEY_ID_ENV_KEY, AWS_SECRET_ACCESS_KEY_ENV_KEY, AWS_SESSION_TOKEN_ENV_KEY, AWS_BEARER_TOKEN_BEDROCK_ENV_KEY, BEDROCK_AWS_ACCESS_KEY_ID_ENV_KEY, BEDROCK_AWS_SECRET_ACCESS_KEY_ENV_KEY, OPENAI_CHATGPT_EMAIL_ENV_KEY, OPENAI_CHATGPT_PLAN_ENV_KEY, } from "../../config/constants.js";
import { openWikiEnvPath } from "../../config/env.js";
import { formatChatGptAccount, } from "../../agent/openai-chatgpt-oauth.js";
import { isCredentialConfigured } from "./steps.js";
export function getAwsCredentialRepairMessage(provider) {
    ...
    const pair = missingEnvKey === BEDROCK_AWS_ACCESS_KEY_ID_ENV_KEY ||
...
export function getCredentialSetupDetail(provider, tokens = null) {
    ...
    if (providerUsesAwsSdkCredentials(provider)) {
        ...
        const missingEnvKey = getMissingProviderEnvKey(provider);
        if (missingEnvKey) {
            if (missingEnvKey === BEDROCK_AWS_ACCESS_KEY_ID_ENV_KEY ||
                missingEnvKey === BEDROCK_AWS_SECRET_ACCESS_KEY_ENV_KEY) {
        ...
        const usesLegacyKeys = Boolean(legacyApiKey &&
            legacySecretKey &&
            process.env[legacyApiKey]?.trim() &&
            process.env[legacySecretKey]?.trim());
        const ignoresOrphanSessionToken = Boolean(process.env[AWS_SESSION_TOKEN_ENV_KEY]?.trim() &&
...
export function openLoginUrl(url) {
    try {
...
export function getOAuthAuthorizationStatusText({ authProvider, copiedToClipboard, }) {
...
export function formatSecretInputDisplay(value) {
...
export function formatTerminalHyperlink(url, label) {
    return `\u001B]8;;${url}\u0007${label}\u001B]8;;\u0007`;
}
export function getSingleLineInputDisplayValue(value, maxLength) {
    if (maxLength <= 0) {
        return "";
    }
