...
import { DEFAULT_PROVIDER, DEFAULT_VERTEX_LOCATION, getDefaultModelId, getProviderApiKeyEnvKey, getProviderBaseUrlEnvKey, getProviderBaseUrlWarnings, getProviderLocationEnvKey, getProviderProjectEnvKey, getProviderRegionEnvKey, getProviderRegionEnvKeys, getProviderSecretKeyEnvKey, OPENWIKI_MODEL_ID_ENV_KEY, OPENWIKI_PROVIDER_ENV_KEY, OPENWIKI_REASONING_EFFORT_ENV_KEY, providerUsesExternalCliAuth, resolveConfiguredProvider, resolveProviderRegion, SELECTABLE_OPENWIKI_PROVIDERS, } from "../../config/constants.js";
import { loginWithChatGPT, } from "../../agent/openai-chatgpt-oauth.js";
import { loadLangSmithSetup, nextLangSmithApiKeyEnv, saveLangSmithSetup, } from "../../connectors/sources/langsmith/setup.js";
import { detectExternalCliCredential, isExternalCliAvailable, runExternalCliLogin, } from "../../auth/external-cli-auth.js";
import { getConnectorConfigPath } from "../../config/openwiki-home.js";
import { getSavedEnvValue, saveOpenWikiEnv } from "../../config/env.js";
import { createEmptyOnboardingConfig, isOnboardingComplete, readOpenWikiOnboardingConfig, saveRepositoryWikiInstructions, saveOpenWikiOnboardingConfig, } from "../onboarding.js";
import { getSuggestedCronExpression, installOpenWikiPowerSchedule, installConnectorSchedule, validateCronExpression, } from "../../scheduling/schedules.js";
import { addSourceInstanceConfig, createSourceInstanceId, createSourceInstanceName, ensureRunModeConfig, getConfigModeId, getConnectedSourceCount, getDefaultCodeRepoRootPath, getDefaultLocalGitRepoPath, getErrorMessage, getInitialStep, getInputDisplayWidth, getLangsmithRegionSelectionIndex, getModelSelectionIndex, getModelSelectionOptions, getReasoningEffortSelectionIndex, getReasoningEffortSelectionOptions, getNextStepAfterApiKey, getNextStepAfterBaseUrl, getNextStepAfterGcpLocation, getNextStepAfterProvider, getNextStepAfterRegion, getNextStepAfterSecretKey, getProviderSelectionIndex, getRunModeSelectionIndex, getSelectedModelId, getSourceDescriptionOptionCount, getSourceOption, getStaticSourceConfig, getTemplateGoal, getTemplateSourceOptions, handleCronEditorInput, hydrateRunModeConfig, isCodeMode, isCredentialConfigured, isSecretKeyConfigured, moveSelectionIndex, needsEnvValue, needsLangSmithStep, nextSetupStep, normalizeLocalPath, sanitizeInputChunk, sanitizeRepoId, shouldStartWithCustomModelInput, validateLocalDirectoryPath, } from "./steps.js";
import { copyToClipboard, getAwsCredentialRepairMessage, openLoginUrl, } from "./format.js";
import { CODE_REPO_OPTIONS, CRON_MODE_OPTIONS, FINAL_OPTIONS, LANGSMITH_REGION_OPTIONS, ONBOARDING_TEMPLATES, POWER_MODE_OPTIONS, RUN_MODE_OPTIONS, SOURCE_CONTINUE_OPTIONS, } from "./constants.js";
 ...
 */
export function useInitSetup({ allowModeSelection = false, mode, modelIdOverride = null, onComplete, onError, walkAllSteps = false, }) {
    ...
    const [gcpProject, setGcpProject] = useState(null);
    ...
    const [langsmithDraft, setLangsmithDraft] = useState(null);
    ...
    const [langsmithWorkspaceSelectionIndex, setLangsmithWorkspaceSelectionIndex,] = useState(0);
    const [langsmithRegionSelectionIndex, setLangsmithRegionSelectionIndex] = useState(0);
    ...
    const [reasoningEffortSelectionIndex, setReasoningEffortSelectionIndex] = useState(0);
    ...
    const [externalCliAuth, setExternalCliAuth] = useState({
        kind: "idle",
    });
    const externalCliProbeProvider = useRef(null);
    ...
    useEffect(() => {
        ...
        readOpenWikiOnboardingConfig()
            ...
            const initialStep = getInitialStep(modelIdOverride, initialProvider, configForMode, mode, allowModeSelection, walkAllSteps);
            if (initialStep === null) {
                onComplete({
                    mode,
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                const nextStep = nextSetupStep("oauth-login", provider, selectedMode, allowModeSelection) ??
                    getNextStepAfterApiKey(provider, modelIdOverride, onboardingConfig, selectedMode, forceModelStep);
                if (nextStep) {
     ...
     */
    function seedInputForStep(target) {
        switch (target) {
            case "provider":
            ...
            case "source-langsmith-region":
                setLangsmithRegionSelectionIndex(getLangsmithRegionSelectionIndex(langsmithDraft?.region ?? "us"));
                break;
            case "source-langsmith-key":
                ...
                setInput(langsmithDraft?.apiKey ||
                    (langsmithDraft
            ...
            case "model": {
                ...
                setInput(shouldStartWithCustomModelInput(provider)
                    ? (modelId ??
                        modelIdOverride ??
                        getSavedEnvValue(OPENWIKI_MODEL_ID_ENV_KEY) ??
                        "")
            ...
            case "reasoning-effort": {
                ...
                const selectedEffort = reasoningEffort ??
                    ...
                    "";
                setReasoningEffortSelectionIndex(getReasoningEffortSelectionIndex(provider, selectedModelId, selectedEffort));
                break;
            }
            case "api-key": {
            ...
            case "gcp-project": {
                ...
                setInput(gcpProject ?? (envKey ? (getSavedEnvValue(envKey) ?? "") : ""));
                break;
            }
            case "gcp-location": {
                const envKey = getProviderLocationEnvKey(provider);
                setInput(gcpLocation ?? (envKey ? (getSavedEnvValue(envKey) ?? "") : ""));
                break;
            }
            case "langsmith":
                ...
                break;
            case "template":
                setTemplateSelectionIndex(Math.max(0, ONBOARDING_TEMPLATES.findIndex((template) => template.id === getConfigModeId(onboardingConfig))));
                setInput("");
                break;
            case "wiki-goal":
            ...
            case "global-cron-custom":
                setInput(onboardingConfig.ingestionSchedule?.expression ??
                    suggestedCronExpression);
     ...
     */
    function captureInputForStep(from) {
        ...
        switch (from) {
            ...
            case "gcp-project":
                if (trimmed)
                    setGcpProject(trimmed);
                break;
            case "gcp-location":
                if (trimmed)
                    setGcpLocation(trimmed);
                break;
            case "langsmith":
            ...
            case "source-langsmith-key":
                ...
                setLangsmithDraft((draft) => draft ? { ...draft, apiKey: trimmed } : draft);
                break;
            case "source-langsmith-projects":
                // Keep an unsubmitted list edit on the draft so Esc does not lose it.
                setLangsmithDraft((draft) => draft
                    ? {
                        ...draft,
                        projects: [
                            ...new Set(input
    ...
    useInput((inputValue, key) => {
        ...
        if (step === "reasoning-effort") {
            const selectedModelId = modelId ?? modelIdOverride ?? getDefaultModelId(provider);
            const optionCount = getReasoningEffortSelectionOptions(provider, selectedModelId).length;
            handleMenuInput(key, () => setReasoningEffortSelectionIndex((index) => moveSelectionIndex(index, key.upArrow ? -1 : 1, optionCount)));
            return;
        }
        if (step === "run-mode") {
        ...
        if (step === "source-langsmith-workspaces") {
            handleMenuInput(key, () => setLangsmithWorkspaceSelectionIndex((index) => moveSelectionIndex(index, key.upArrow ? -1 : 1, 
            // workspaces + "Add a workspace" + "Done".
            langsmithWorkspaces.length + 2)));
            return;
        }
        if (step === "source-langsmith-region") {
            handleMenuInput(key, () => setLangsmithRegionSelectionIndex((index) => moveSelectionIndex(index, key.upArrow ? -1 : 1, LANGSMITH_REGION_OPTIONS.length)));
            return;
        }
    ...
    async function submit() {
        ...
        if (step === "provider") {
            ...
            const nextStep = nextSetupStep("provider", selectedProvider, selectedMode, allowModeSelection) ??
                getNextStepAfterProvider(selectedProvider, modelIdOverride, onboardingConfig, selectedMode, providerChanged);
            if (nextStep) {
        ...
        if (step === "api-key" || step === "external-cli-auth") {
            ...
            const nextApiKey = trimmedInput.length > 0
                ? trimmedInput
            ...
            if (nextApiKey === null &&
                ...
                setError(`${getProviderApiKeyEnvKey(provider) ?? "API key"} is required.`);
                return;
            }
            ...
            setInput("");
            const nextStep = nextSetupStep(step, provider, selectedMode, allowModeSelection) ??
                getNextStepAfterApiKey(provider, modelIdOverride, onboardingConfig, selectedMode, forceModelStep);
            if (nextStep) {
                setIsCustomModelInput(nextStep === "model" && shouldStartWithCustomModelInput(provider));
                seedInputForStep(nextStep);
        ...
        if (step === "secret-key") {
            ...
            if (nextSecretKey === null && !isSecretKeyConfigured(provider)) {
                setError(`${getProviderSecretKeyEnvKey(provider) ?? "Secret key"} is required.`);
                return;
            }
            if (trimmedInput.length > 0) {
            ...
            setInput("");
            const nextStep = nextSetupStep("secret-key", provider, selectedMode, allowModeSelection) ??
                getNextStepAfterSecretKey(provider, modelIdOverride, onboardingConfig, selectedMode, forceModelStep);
            if (nextStep) {
                setIsCustomModelInput(nextStep === "model" && shouldStartWithCustomModelInput(provider));
                seedInputForStep(nextStep);
                setStep(nextStep);
        ...
        if (step === "region") {
            ...
            if (trimmedInput.length === 0 && !configuredRegion) {
                ...
                setError(`Set one of ${regionEnvKeys.join(", ") || "the supported region variables"}.`);
                return;
            }
            ...
            setInput("");
            const nextStep = nextSetupStep("region", provider, selectedMode, allowModeSelection) ??
                getNextStepAfterRegion(provider, modelIdOverride, onboardingConfig, selectedMode, forceModelStep);
            if (nextStep) {
                setIsCustomModelInput(nextStep === "model" && shouldStartWithCustomModelInput(provider));
                seedInputForStep(nextStep);
                setStep(nextStep);
        ...
        if (step === "gcp-project") {
            const trimmedInput = input.trim();
            if (trimmedInput.length === 0) {
                setError(`${getProviderProjectEnvKey(provider) ?? "GCP project"} is required.`);
                return;
            }
        ...
        if (step === "gcp-location") {
            const trimmedInput = input.trim();
            if (/\s/u.test(trimmedInput)) {
                setError(`Enter a valid location (no spaces), or leave blank for ${DEFAULT_VERTEX_LOCATION}.`);
                return;
            }
            const nextGcpLocation = trimmedInput.length > 0 ? trimmedInput : null;
            setGcpLocation(nextGcpLocation);
            setInput("");
            const nextStep = nextSetupStep("gcp-location", provider, selectedMode, allowModeSelection) ??
                getNextStepAfterGcpLocation(provider, modelIdOverride, onboardingConfig, selectedMode, forceModelStep);
            if (nextStep) {
                setIsCustomModelInput(nextStep === "model" && shouldStartWithCustomModelInput(provider));
                seedInputForStep(nextStep);
                setStep(nextStep);
        ...
        if (step === "base-url") {
            ...
            const baseUrlWarnings = getProviderBaseUrlWarnings(provider, trimmedInput);
            ...
            setInput("");
            const nextStep = nextSetupStep("base-url", provider, selectedMode, allowModeSelection) ??
                getNextStepAfterBaseUrl(provider, modelIdOverride, onboardingConfig, selectedMode, forceModelStep);
            if (nextStep) {
                setIsCustomModelInput(nextStep === "model" && shouldStartWithCustomModelInput(provider));
                seedInputForStep(nextStep);
                setStep(nextStep);
        ...
        if (step === "reasoning-effort") {
            const selectedModelId = modelId ?? modelIdOverride ?? getDefaultModelId(provider);
            const selectedEffort = getReasoningEffortSelectionOptions(provider, selectedModelId)[reasoningEffortSelectionIndex]?.value ?? "";
        ...
        if (step === "template") {
            ...
            const keepExistingGoal = selectedTemplate.id === getConfigModeId(onboardingConfig) &&
                onboardingConfig.wikiGoal !== undefined &&
                onboardingConfig.wikiGoal.length > 0;
            setInput(keepExistingGoal
        ...
        if (step === "source-langsmith-workspaces") {
            ...
            if (langsmithWorkspaceSelectionIndex < workspaceCount) {
                ...
                setLangsmithRegionSelectionIndex(getLangsmithRegionSelectionIndex(existing.region));
            ...
            if (langsmithWorkspaceSelectionIndex === workspaceCount) {
                ...
                setLangsmithDraft({
                    apiKey: "",
                    apiKeyEnv: nextLangSmithApiKeyEnv(langsmithWorkspaces.map((workspace) => workspace.apiKeyEnv)),
                    projects: [],
                    region: "us",
                });
                setLangsmithRegionSelectionIndex(getLangsmithRegionSelectionIndex("us"));
                setStep("source-langsmith-region");
                return;
            }
            // Done.
        ...
        if (step === "source-langsmith-region") {
            const selectedOption = LANGSMITH_REGION_OPTIONS[langsmithRegionSelectionIndex] ??
                LANGSMITH_REGION_OPTIONS[0];
            setLangsmithDraft((draft) => draft ? { ...draft, region: selectedOption.id } : draft);
        ...
        if (step === "source-langsmith-key") {
            ...
            setLangsmithDraft((draft) => draft ? { ...draft, apiKey: nextKey } : draft);
        ...
        if (step === "final") {
            ...
            if (selectedMode === "code" && langsmithSourcesTouched) {
                try {
                    ...
                    await saveLangSmithSetup(codeRepoRoot, langsmithWorkspaces.map((workspace) => ({
    ...
    async function continueAfterModel(nextModelId, nextReasoningEffort) {
    ...
    function continueAfterCodeRepoConfirmed(repoRoot) {
        ...
        if (!langsmithPreloadedRef.current) {
            ...
            void loadLangSmithSetup(repoRoot)
                .then((existing) => setLangsmithWorkspaces(existing.map((workspace) => ({
                apiKey: "",
                apiKeyEnv: workspace.apiKeyEnv,
                projects: workspace.projects,
                region: workspace.region,
    ...
    async function saveCredentialUpdates(options) {
        setIsSaving(true);
        try {
            const updates = buildCredentialEnvUpdates({
                ...
                nextOAuthTokens: options.nextOAuthTokens === undefined
                    ? oauthTokens
     ...
     */
    function commitLangsmithWorkspace(names) {
