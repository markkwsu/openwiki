 ...
 */
export async function findInvalidMermaidFences(markdown) {
 ...
 */
export function heuristicError(body) {
 ...
 */
export async function degradeInvalidMermaidFences(markdown) {
    ...
    for (const { fence, error } of [...errors].reverse()) {
        const comment = `${fence.indent}<!-- openwiki: mermaid parse failed and this diagram ` +
        ...
        lines.splice(fence.openLine, fence.closeLine - fence.openLine + 1, comment, `${fence.indent}${fence.marker}text`, ...fence.body.split("\n"), `${fence.indent}${fence.marker}`);
    }
 ...
 * `findInvalidMermaidFences`.
 */
export function sanitizeMermaidError(error) {
