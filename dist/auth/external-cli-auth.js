...
import { getProviderApiKeyEnvKey, getProviderBaseUrlEnvKey, getProviderExternalCliAuthAdapter, providerUsesExternalCliAuth, } from "../config/constants.js";
 ...
 */
function resolveGithubCliHostname(provider, env) {
...
function buildExternalCliAuthAdapters(provider, env) {
    ...
    return {
        "github-cli": {
            ...
            installHint: "Install the GitHub CLI (https://cli.github.com), then run `gh auth login`.",
...
export function getExternalCliAuthAdapter(provider, env = process.env) {
    const adapterId = getProviderExternalCliAuthAdapter(provider);
    return adapterId
        ? buildExternalCliAuthAdapters(provider, env)[adapterId]
        : null;
}
export async function isExternalCliAvailable(provider) {
...
export async function detectExternalCliCredential(provider, env = process.env) {
 ...
 * env file.
 */
export async function resolveExternalCliCredential(provider, env = process.env) {
...
export function runExternalCliLogin(provider) {
    const adapter = getExternalCliAuthAdapter(provider);
    if (!adapter) {
...
export function validateExternalCliCredential(provider, credential) {
    if (getProviderExternalCliAuthAdapter(provider) === "github-cli" &&
        /^(ghp_|github_pat_)/u.test(credential)) {
        throw new Error("The GitHub Copilot API does not accept Personal Access Tokens. Authenticate with `gh auth login` using a Copilot-enabled account, or set COPILOT_API_KEY to a GitHub OAuth token.");
    }
}
