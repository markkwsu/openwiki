...
export async function discoverProtectedResourceMetadata(resourceUrl, options = {}) {
    const url = validateOAuthEndpointUrl(resourceUrl, "MCP protected resource URL", options);
    const candidates = [
        `${url.origin}/.well-known/oauth-protected-resource${url.pathname}`,
        `${url.origin}/.well-known/oauth-protected-resource`,
    ];
    for (const candidate of candidates) {
        const response = await fetch(validateOAuthEndpointUrl(candidate, "MCP protected resource metadata", options), { redirect: "manual" });
        if (response.ok) {
...
export async function discoverAuthorizationServerMetadata(issuer, options = {}) {
    const issuerUrl = validateOAuthEndpointUrl(issuer, "OAuth authorization server issuer", options);
    const candidates = [
    ...
    for (const candidate of candidates) {
        const response = await fetch(validateOAuthEndpointUrl(candidate, "OAuth authorization server metadata", options), { redirect: "manual" });
        if (response.ok) {
            return (await response.json());
        }
    }
...
export function validateOAuthEndpointUrl(value, label, options = {}) {
