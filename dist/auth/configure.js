...
import { ensureConnectorHome, getConnectorConfigPath, openWikiEnvDisplayPath, } from "../config/openwiki-home.js";
import { discoverMcpConnectorTools, isMcpConnectorId, } from "../connectors/mcp-runtime.js";
export async function configureAuthProvider(provider, options = {}) {
    ...
    await writeFile(configPath, `${JSON.stringify(getDefaultConfig(provider), null, 2)}\n`, {
        encoding: "utf8",
        mode: 0o600,
    });
...
export async function listAuthProviderTools(provider) {
    const connectorId = getConnectorIdForProvider(provider);
    await ensureConnectorHome(connectorId);
    const configPath = getConnectorConfigPath(connectorId);
    const existing = await readExistingConfig(configPath);
    if (existing === null) {
        throw new Error(`Connector config does not exist. Run openwiki auth ${provider} first.`);
    }
...
export function shouldDiscoverToolsAfterAuth(provider) {
    return isMcpConnectorId(getConnectorIdForProvider(provider));
}
function getConnectorIdForProvider(provider) {
...
function getNextSteps(provider, wroteConfig) {
