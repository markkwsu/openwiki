...
async function registerMcpOAuthClient(provider, redirectUri) {
    ...
    const authMetadata = await discoverAuthorizationServerMetadata(authServer, validationOptions);
    ...
    const authorizationEndpoint = validateOAuthEndpointUrl(authMetadata.authorization_endpoint, `${provider.displayName} authorization endpoint`, validationOptions).toString();
    const tokenEndpoint = validateOAuthEndpointUrl(authMetadata.token_endpoint, `${provider.displayName} token endpoint`, validationOptions).toString();
    const registrationEndpoint = validateOAuthEndpointUrl(authMetadata.registration_endpoint, `${provider.displayName} registration endpoint`, validationOptions).toString();
...
function createAuthorizationUrl(provider, registration, redirectUri, state, codeChallenge) {
    const authUrl = validateOAuthEndpointUrl(registration.authUrl, `${provider.displayName} authorization endpoint`);
...
async function exchangeAuthorizationCode({ code, codeVerifier, provider, redirectUri, registration, }) {
    ...
    const response = await fetch(validateOAuthEndpointUrl(registration.tokenUrl, `${provider.displayName} token endpoint`).toString(), {
        body,
        headers: {
...
export async function createCallbackServer(provider) {
    ...
    const server = http.createServer((request, response) => {
        ...
        const requestUrl = new URL(request.url ?? "/", `http://${CALLBACK_HOST}:${callbackPort}`);
