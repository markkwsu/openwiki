...
import { createOpenWikiContentSnapshot, createRepositorySourceSnapshot, createRunContext, getRepositoryChangedPaths, getUpdateNoopStatus, persistRunMetadataIfChanged, writeLastUpdateMetadata, } from "../agent/utils.js";
import { deserializePreparedWikiState, finalizeWikiArtifacts, prepareWikiForAuthoring, serializePreparedWikiState, } from "../agent/wiki-finalizer.js";
import { beginRepositoryWikiReplacement } from "../agent/wiki-replacement.js";
import { prepareClaimsRuntime, } from "../claims/brains/code/runtime.js";
import { ClaimsStore } from "../claims/brains/code/store.js";
import { ensureCodeModeRepoSetup } from "../ingestion/code-mode.js";
import { parseFrontmatterFields, repairPersistedFile, } from "../okf/frontmatter.js";
import { resolveConceptTypeLabel, resolveIndexLabels, } from "../okf/index-labels.js";
import { getPrimaryLanguageSubtag, requireResolvedLanguage, resolveLanguage, } from "../platform/language.js";
import { isFileNotFoundError } from "../platform/fs-errors.js";
import { RepositoryRunError } from "./errors.js";
import { getCurrentRepositoryPageCompletion, readRepositoryPageManifest, recordRepositoryPageCompletion, replaceRepositoryPageManifest, seedRepositoryPageManifest, } from "./page-manifest.js";
import { createRepositoryPlan, reconcilePageClaims, } from "./page-jobs.js";
import { readRepositoryRunState, removeRepositoryRunState, writeRepositoryRunState, } from "./run-state.js";
/**
 ...
 */
export function getRepositoryRunSourceCheckpoint(state) {
    return {
 ...
 */
export async function beginRepositoryRun(input) {
    ...
    const requestedLanguage = resolvedRequest.kind === "resolved" ? resolvedRequest.language : undefined;
    ...
    const languageChanged = requestedLanguage !== undefined &&
    ...
    const replacement = input.mode === "init"
    ...
    try {
        ...
        const claimsRuntime = await prepareClaimsRuntime(input.mode, "repository", input.root, ignore, () => undefined, { resumeInit: false });
        if (!claimsRuntime) {
        ...
        if (input.mode === "update" &&
            ...
            await seedRepositoryPageManifest(input.root, initialPages, context.lastUpdate.gitHead);
        }
        if (input.mode === "update") {
            await fastForwardUnchangedRepositoryPageCoverage(input.root, ignore, initialPages, {
                sourceFingerprint: source.fingerprint,
                ...(source.gitHead ? { gitHead: source.gitHead } : {}),
            });
        }
        const seededManifest = input.mode === "update"
            ? await readRepositoryPageManifest(input.root)
            : undefined;
        const hasCompleteBaselineCoverage = input.mode !== "update" ||
        ...
        if (input.mode === "update" && input.force !== true) {
            const preflight = await getUpdateNoopStatus(input.root, ignore, input.language);
            if (preflight.shouldSkip &&
                claimsRuntime.issueCount === 0 &&
                hasCompleteBaselineCoverage) {
                const source = await createRepositorySourceSnapshot(input.root, ignore);
                await claimsRuntime.finalize(now().toISOString());
                const stableSource = await createRepositorySourceSnapshot(input.root, ignore);
                if (stableSource.fingerprint === source.fingerprint) {
                    await replaceRepositoryPageManifest(input.root, initialPages, {
                        sourceFingerprint: source.fingerprint,
                        ...(source.gitHead ? { gitHead: source.gitHead } : {}),
                    });
                    const publishedSource = await createRepositorySourceSnapshot(input.root, ignore);
                    if (publishedSource.fingerprint === source.fingerprint) {
                        await writeLastUpdateMetadata("update", input.root, preflight.model, "repository", "complete", preflight.language);
                        return {
                            view: {
                                status: "noop",
                                root: input.root,
        ...
        const beforeContentSnapshot = await createOpenWikiContentSnapshot(input.root, "repository");
        const preparedWiki = await prepareWikiForAuthoring({
            backend,
            outputMode: "repository",
            conceptType: resolveConceptTypeLabel(language),
        });
        const requiredRewritePages = input.mode === "update" && languageChanged ? initialPages : [];
        const state = {
            schemaVersion: 1,
            runId: randomUUID(),
            mode: input.mode,
        ...
        await writeLastUpdateMetadata(input.mode, input.root, input.actor.metadataModel, "repository", "interrupted", language, context.lastUpdate?.gitHead ?? null);
 ...
 */
async function resumeRepositoryRun(input, state) {
    let activeState = state;
    if (state.mode !== input.mode) {
        throw new RepositoryRunError("conflict", `An interrupted OpenWiki ${state.mode} run already exists. Resume that run before starting ${input.mode}.`);
    }
    const requestedLanguage = requireResolvedLanguage(input.language);
    if (requestedLanguage && requestedLanguage !== state.language) {
        throw new RepositoryRunError("conflict", `Interrupted OpenWiki run uses ${state.language}; resume it before changing the wiki language to ${requestedLanguage}.`);
    }
    const ignore = await OpenWikiIgnore.load(input.root);
    ...
    const resetSkippedPages = state.plan?.pages.some(({ status }) => status === "skipped") ?? false;
    let nextState = {
        ...state,
        actor: { ...input.actor },
        ...(state.plan && resetSkippedPages
            ? {
                plan: {
                    ...state.plan,
                    pages: state.plan.pages.map((page) => page.status === "skipped"
                        ? { ...page, status: "pending" }
                        : page),
                },
            }
    ...
    if (sourceChanged) {
        ...
        nextState = await reconcileManifestPageJobs(input.root, nextState, state.actor.producerActor);
    }
    ...
    const backend = createRepositoryBackend(input.root, ignore);
    const claimsRuntime = await prepareClaimsRuntime(activeState.mode, "repository", input.root, ignore, () => undefined, { resumeInit: activeState.mode === "init" });
    if (!claimsRuntime) {
        throw new Error("Repository Claims runtime was not prepared.");
    }
    const run = {
 ...
 */
async function reconcileManifestPageJobs(root, state, legacyCompletedBy) {
    if (!state.plan)
        return state;
    ...
    for (const page of state.plan.pages) {
        let current = await getCurrentRepositoryPageCompletion(root, page.path, source);
        if (page.status === "complete" && !current) {
            ...
            current = await recordRepositoryPageCompletion(root, page.path, source, page.completedBy ?? legacyCompletedBy, state.runId);
        }
 ...
 */
function createRepositoryBackend(root, ignore) {
 ...
 */
async function toActiveBeginView(run, resumed) {
    const pages = run.state.plan?.pages ?? [];
    const pageUpdateWindows = run.state.mode === "update"
        ? await getRepositoryPageUpdateWindows(run.root, run.ignore, run.state.initialPages)
        : [];
    const changedPaths = [
 ...
 */
async function getRepositoryPageUpdateWindows(root, ignore, pages) {
    ...
    for (const baseline of baselines) {
        windows.push({
            ...
            changedPaths: await getRepositoryChangedPaths(root, ignore, baseline || undefined),
 ...
 */
async function fastForwardUnchangedRepositoryPageCoverage(root, ignore, pages, source) {
    if (!source.gitHead)
        return;
    const manifest = await readRepositoryPageManifest(root);
    for (const page of [...pages].sort(compareCodeUnits)) {
        const entry = manifest.pages[page];
        if (!entry?.gitHead || entry.gitHead === source.gitHead)
            continue;
        const changedPaths = await getRepositoryChangedPaths(root, ignore, entry.gitHead);
        if (changedPaths.length > 0)
            continue;
        await recordRepositoryPageCompletion(root, page, source, entry.completedBy, entry.completedRunId);
    }
}
/**
 ...
 */
export async function submitRepositoryPlan(run, input) {
    if (run.state.plan) {
        ...
        const proposed = createRepositoryPlan(run.state.mode, input, run.claimsRuntime.issues, run.state.requiredRewritePages);
        if (!samePlanIgnoringJobIds(run.state.plan, proposed)) {
            throw new RepositoryRunError("invalid_state", "This OpenWiki run already has a different persisted plan.");
        }
        return { status: "accepted", totalPages: run.state.plan.pages.length };
    }
    if (run.state.phase !== "planning") {
        throw new RepositoryRunError("invalid_state", "OpenWiki plan can only be submitted during planning.");
    }
    const plan = createRepositoryPlan(run.state.mode, input, run.claimsRuntime.issues, run.state.requiredRewritePages);
    const nextState = {
        ...run.state,
        phase: "generating",
        plan,
 ...
 */
function samePlanIgnoringJobIds(left, right) {
    const simplify = (plan) => ({
        pages: plan.pages.map(({ path, title, purpose, seedPaths, relatedPages, instructions }) => ({
            path,
            title,
            purpose,
            seedPaths,
 ...
 */
export async function nextRepositoryPage(run) {
    const plan = run.state.plan;
    if (!plan || run.state.phase !== "generating") {
        throw new RepositoryRunError("invalid_state", "Submit the OpenWiki plan before requesting page work.");
    }
 ...
 */
export function inspectRepositoryPageClaims(run, jobId) {
    const current = run.state.plan?.pages.find(({ status }) => status === "pending");
    if (!current || current.id !== jobId) {
        throw new RepositoryRunError("invalid_state", "Only the current pending OpenWiki page job's Claims may be inspected.");
    }
    return {
        page: current.path,
 ...
 */
export async function captureRepositoryPageSnapshot(run, jobId) {
    const current = run.state.plan?.pages.find(({ status }) => status === "pending");
    if (!current || current.id !== jobId) {
        throw new RepositoryRunError("invalid_state", "Only the current pending OpenWiki page job may be snapshotted.");
    }
    let markdown = null;
    try {
        const read = await run.backend.readRaw(current.path);
        if (read.error && !isNotFoundBackendError(read.error)) {
            throw new RepositoryRunError("invalid_state", `Could not snapshot ${current.path}: ${read.error}`);
        }
        const content = read.data?.content;
        if (content !== undefined && typeof content !== "string") {
            throw new RepositoryRunError("invalid_state", `Could not snapshot non-text Markdown page ${current.path}.`);
        }
 ...
 */
export async function skipRepositoryPage(run, snapshot) {
    const plan = run.state.plan;
    ...
    if (!plan ||
        ...
        throw new RepositoryRunError("invalid_state", "The failed page worker no longer owns the current pending job.");
    }
    ...
    const claimsRuntime = await prepareClaimsRuntime(run.state.mode, "repository", run.root, run.ignore, () => undefined, { resumeInit: run.state.mode === "init" });
    if (!claimsRuntime) {
        throw new Error("Repository Claims runtime was not restored.");
    }
    run.claimsRuntime = claimsRuntime;
    await writeLastUpdateMetadata(run.state.mode, run.root, run.state.actor.metadataModel, "repository", "interrupted", run.state.language, run.state.baseGitHead ?? null);
    const nextState = {
        ...run.state,
        plan: {
            ...plan,
            pages: plan.pages.map((page) => page.id === snapshot.jobId
                ? { ...page, status: "skipped" }
                : page),
        },
    };
...
async function restoreRepositoryPageMarkdown(run, snapshot) {
    const result = snapshot.markdown === null
 ...
 */
export async function submitRepositoryPage(run, input) {
    const plan = run.state.plan;
    if (!plan || run.state.phase !== "generating") {
        throw new RepositoryRunError("invalid_state", "Submit the OpenWiki plan before submitting pages.");
    }
    ...
    if (!current || current.id !== requested.id) {
        throw new RepositoryRunError("invalid_state", "Only the current pending OpenWiki page job may be submitted.");
    }
    ...
    if (!pageReadable) {
        throw new RepositoryRunError("invalid_input", `Write ${current.path} before submitting its page job.`);
    }
    const frontmatter = await repairPersistedFile(run.backend, current.path, resolveConceptTypeLabel(run.state.language));
    if (!frontmatter.validation.valid) {
        const details = frontmatter.validation.issues
            ...
            .join("; ");
        throw new RepositoryRunError("invalid_input", `Could not deterministically repair front matter in ${current.path}: ${details}`);
    }
    try {
        ...
        if (error instanceof RepositoryRunError)
            throw error;
        throw new RepositoryRunError("invalid_input", error instanceof Error ? error.message : "Claims validation failed.");
    }
    await recordRepositoryPageCompletion(run.root, current.path, getRepositoryRunSourceCheckpoint(run.state), run.state.actor.producerActor, run.state.runId);
    const nextPlan = {
        ...plan,
        pages: plan.pages.map((page) => page.id === current.id
            ? {
                ...page,
                status: "complete",
 ...
 */
async function assertPageClaimsDurable(run, page, retryOperation = "submit_page") {
    const store = new ClaimsStore(run.root);
    const persisted = await store.loadPage(page);
    if (!persisted) {
        throw new RepositoryRunError("invalid_state", `Claims for ${page} were not durably persisted; retry ${retryOperation}.`);
    }
    const currentPageVersion = await store.hashPage(page);
    if (persisted.pageVersion !== currentPageVersion) {
        throw new RepositoryRunError("invalid_state", `Claims for ${page} do not match the current Markdown bytes; retry ${retryOperation}.`);
    }
    if (!persisted.verification) {
        throw new RepositoryRunError("invalid_state", `Claims for ${page} were not verified; retry ${retryOperation}.`);
    }
    const verified = parseFrontmatterFields(await store.readMarkdown(page))?.verified;
    const verificationEvents = Array.isArray(verified)
        ? verified
        : verified === undefined
            ? []
            : [verified];
    const verificationProjected = verificationEvents.some((event) => isVerificationEvent(event) &&
        event.by === persisted.verification?.by &&
        event.at === persisted.verification?.at);
    if (!verificationProjected) {
        throw new RepositoryRunError("invalid_state", `Claims verification for ${page} was not durably projected; retry ${retryOperation}.`);
    }
    const expected = run.claimsRuntime.session.inspectClaims(page);
    const persistedById = new Map(persisted.claims.map((claim) => [claim.id, claim]));
    if (persistedById.size !== expected.length) {
        throw new RepositoryRunError("invalid_state", `Claims for ${page} were only partially persisted; retry ${retryOperation}.`);
    }
    for (const claim of expected) {
        const durable = persistedById.get(claim.id);
        if (!durable ||
            durable.statement !== claim.statement ||
            !sameResourceSet(durable.evidence.map(({ resource }) => resource), claim.evidence)) {
            throw new RepositoryRunError("invalid_state", `Claims for ${page} were only partially persisted; retry ${retryOperation}.`);
        }
    }
}
 ...
 */
function isVerificationEvent(value) {
    return (value !== null &&
        typeof value === "object" &&
        !Array.isArray(value) &&
        "by" in value &&
 ...
 */
async function assertRepositoryClaimsDurable(run, excludedPages = new Set()) {
    const store = new ClaimsStore(run.root);
    ...
    for (const page of sidecarPages) {
        if (!currentPages.has(page)) {
            throw new RepositoryRunError("invalid_state", `Deleted or missing page ${page} still has a Claims sidecar; retry finish.`);
        }
    }
    ...
    for (const page of run.claimsRuntime.session
        ...
        if (excludedPages.has(page))
            continue;
        if (run.claimsRuntime.session.inspectClaims(page).length === 0)
            continue;
        if (!currentPages.has(page)) {
            throw new RepositoryRunError("invalid_state", `Claimed page ${page} is missing; retry finish.`);
        }
 ...
 */
function sameResourceSet(left, right) {
    const normalize = (values) => [...new Set(values)].sort(compareCodeUnits);
 ...
 */
export async function finishRepositoryRun(run, options = {}) {
    const plan = run.state.plan;
    if (!plan) {
        throw new RepositoryRunError("invalid_state", "OpenWiki cannot finish before a plan is submitted.");
    }
    const pending = plan.pages.filter(({ status }) => status === "pending");
    if (pending.length > 0) {
        throw new RepositoryRunError("invalid_state", `OpenWiki cannot finish with ${pending.length} pending page job(s).`);
    }
    const skippedJobs = plan.pages.filter(({ status }) => status === "skipped");
    const snapshots = options.skippedPageSnapshots ?? [];
    const snapshotsByJobId = new Map(snapshots.map((snapshot) => [snapshot.jobId, snapshot]));
    if (skippedJobs.length !== snapshots.length ||
        skippedJobs.some((job) => snapshotsByJobId.get(job.id)?.path !== job.path)) {
        throw new RepositoryRunError("invalid_state", "Every skipped OpenWiki page job requires its original page snapshot before finish.");
    }
    const skippedPages = new Set(skippedJobs.map(({ path }) => path));
    const sourceChangedBeforeFinish = await hasRepositorySourceChanged(run);
    const producerActorsByPage = new Map();
    for (const [page, entry] of Object.entries((await readRepositoryPageManifest(run.root)).pages)) {
    ...
    const store = new ClaimsStore(run.root);
    await replaceRepositoryPageManifest(run.root, await store.discoverPages(), getRepositoryRunSourceCheckpoint(run.state), skippedPages);
    const sourceChanged = sourceChangedBeforeFinish || (await hasRepositorySourceChanged(run));
    if (skippedPages.size > 0 || sourceChanged) {
        await writeLastUpdateMetadata(run.state.mode, run.root, run.state.actor.metadataModel, "repository", "interrupted", run.state.language, run.state.baseGitHead ?? null);
    }
    else {
        await persistRunMetadataIfChanged(run.state.mode, run.root, run.state.actor.metadataModel, "repository", run.state.beforeContentSnapshot, "complete", run.state.language);
    }
 ...
 */
async function hasRepositorySourceChanged(run) {
 ...
 */
async function applyAbandonedGeneratedPageDeletions(run, plan) {
    ...
    for (const page of await store.discoverPages()) {
        if (initial.has(page) || planned.has(page))
            continue;
        const result = await run.backend.delete(page);
        if (result.error && !isNotFoundBackendError(result.error)) {
            throw new RepositoryRunError("invalid_state", `Could not remove page abandoned by an invalidated plan ${page}: ${result.error}`);
        }
 ...
 */
async function applyPlannedDeletions(run, pages) {
    for (const page of pages) {
        const result = await run.backend.delete(page);
        if (result.error && !isNotFoundBackendError(result.error)) {
            throw new RepositoryRunError("invalid_state", `Could not delete planned OpenWiki page ${page}: ${result.error}`);
        }
        await run.claimsRuntime.session.recordDeletion(page);
    }
}
 ...
 */
async function reconcileDeletedClaimPages(run) {
    const store = new ClaimsStore(run.root);
    const currentPages = new Set(await store.discoverPages());
