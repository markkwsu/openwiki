...
import { normalizeClaimsToolPagePath, normalizeWikiPagePath, } from "../claims/brains/code/paths.js";
 ...
 */
export function createRepositoryPlan(mode, proposed, claimIssues, requiredRewritePages = []) {
    const pages = proposed.pages.map(normalizePlanPage);
    const deletePages = uniqueSorted((proposed.deletePages ?? []).map(normalizePlanPagePath));
    if (mode === "init" && deletePages.length > 0) {
        throw new RepositoryRunError("invalid_input", "Init plans cannot delete generated pages.");
    }
    if (deletePages.includes("/openwiki/quickstart.md")) {
        throw new RepositoryRunError("invalid_input", "The canonical /openwiki/quickstart.md page cannot be deleted.");
    }
    const pagePaths = new Set();
    for (const page of pages) {
        if (pagePaths.has(page.path)) {
            throw new RepositoryRunError("invalid_input", `Duplicate planned page: ${page.path}`);
        }
        pagePaths.add(page.path);
    }
    for (const deleted of deletePages) {
        if (pagePaths.has(deleted)) {
            throw new RepositoryRunError("invalid_input", `A page cannot be both generated and deleted: ${deleted}`);
        }
    }
    if (mode === "init" && !pagePaths.has("/openwiki/quickstart.md")) {
        throw new RepositoryRunError("invalid_input", "Init plan must include /openwiki/quickstart.md.");
    }
    if (mode === "update") {
    ...
    pages.sort((left, right) => {
        ...
        if (leftQuickstart !== rightQuickstart)
            return leftQuickstart ? 1 : -1;
 ...
 */
function normalizePlanPage(page) {
    const canonicalPath = normalizePlanPagePath(page.path);
    const title = page.title.trim();
    const purpose = page.purpose.trim();
    if (!title || !purpose) {
        throw new RepositoryRunError("invalid_input", `Planned page requires title and purpose: ${canonicalPath}`);
    }
    return {
        id: randomUUID(),
        path: canonicalPath,
        ...
        relatedPages: uniqueSorted((page.relatedPages ?? []).map(normalizePlanPagePath)),
        instructions: uniqueSorted((page.instructions ?? [])
 ...
 */
function normalizePlanPagePath(value) {
    try {
        ...
        throw new RepositoryRunError("invalid_input", `Invalid or reserved OpenWiki page path: ${value}`);
    }
}
/**
 ...
 */
function addRequiredClaimIssueJobs(pages, pagePaths, deletePages, issues) {
    ...
    for (const [page, pageIssues] of grouped) {
        if (pagePaths.has(page) || deletePages.has(page))
            continue;
        pages.push({
            id: randomUUID(),
            path: page,
            title: titleFromPath(page),
            purpose: "Reconcile stale or unresolved Claims and update this page from current repository evidence while preserving unaffected accurate content.",
            seedPaths: uniqueSorted(pageIssues.flatMap((issue) => issue.resources.map(evidenceResourceToSeedPath))),
            relatedPages: [],
            instructions: [],
            status: "pending",
        });
 ...
 */
function addRequiredRewriteJobs(pages, pagePaths, deletePages, requiredPages) {
    for (const pageInput of requiredPages) {
        ...
        pages.push({
            ...
            path: page,
            title: titleFromPath(page),
            purpose: "Rewrite this existing page in the run's target language while preserving every accurate repository-supported fact and reconciling its complete Claim set.",
            seedPaths: [],
            relatedPages: [],
            instructions: [],
            status: "pending",
...
function normalizeSeedPath(value) {
    ...
    if (!trimmed || trimmed.split("/").some((part) => part === "..")) {
        throw new RepositoryRunError("invalid_input", `Invalid seed path: ${value}`);
    }
    return trimmed;
}
/**
 ...
 */
export async function reconcilePageClaims(session, pageInput, proposedInput) {
    const page = normalizeWikiPagePath(pageInput);
    const existing = session.inspectClaims(page);
    const existingById = new Map(existing.map((claim) => [claim.id, claim]));
    const targetedExistingIds = new Set();
    const proposedFingerprints = new Set();
    const operations = [];
    const targetExisting = (rawId, decision) => {
        const id = rawId.trim();
        const current = existingById.get(id);
        if (!current) {
            throw new RepositoryRunError("invalid_input", `Claim ${id || "(empty)"} is not owned by ${page}.`);
        }
        if (targetedExistingIds.has(id)) {
            throw new RepositoryRunError("invalid_input", `Claim ${id} has more than one reconciliation decision for ${page} (${decision}).`);
        }
    ...
    for (const raw of proposedInput.claims ?? []) {
        ...
        if (proposedFingerprints.has(fingerprint)) {
            throw new RepositoryRunError("invalid_input", `Duplicate proposed Claim for ${page}: ${proposed.statement}`);
        }
        ...
        if (proposed.id) {
            ...
            if (sameClaim(current, proposed)) {
                ...
                const evidenceChanged = !sameEvidence(current.evidence, proposed.evidence);
                operations.push({
                    op: "update",
                    id: current.id,
        ...
        const exactExisting = existing.find((candidate) => sameClaim(candidate, proposed));
        if (exactExisting) {
    ...
    for (const current of existing) {
        if (targetedExistingIds.has(current.id))
            continue;
        if (current.issue) {
            throw new RepositoryRunError("invalid_input", `Claim ${current.id} is ${current.issue.kind} and requires an explicit confirm, update, or retract decision.`);
        }
        operations.push({ op: "confirm", id: current.id });
    }
    ...
    if (existing.length - retractedCount + addedCount === 0) {
        throw new RepositoryRunError("invalid_input", `Completed factual page ${page} must retain or establish at least one material Claim.`);
    }
 ...
 */
function normalizeProposedClaim(claim) {
    const statement = claim.statement.trim();
    if (!statement) {
        throw new RepositoryRunError("invalid_input", "Claim statement is empty.");
    }
    const resources = uniqueSorted(claim.evidence.map(({ resource }) => resource.trim()).filter(Boolean));
    if (resources.length === 0) {
        throw new RepositoryRunError("invalid_input", `Claim requires repository evidence: ${statement}`);
    }
    return {
 ...
 */
function sameClaim(existing, proposed) {
    return (existing.statement === proposed.statement &&
 ...
 */
function sameEvidence(existing, proposed) {
 ...
 */
function claimFingerprint(statement, evidence) {
