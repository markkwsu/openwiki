...
export const OPENAI_COMPATIBLE_STREAMING_ENV_KEY = "OPENWIKI_OPENAI_COMPATIBLE_STREAMING";
export const OPENAI_COMPATIBLE_USE_RESPONSES_API_ENV_KEY = "OPENWIKI_OPENAI_COMPATIBLE_USE_RESPONSES_API";
export const OPENAI_COMPATIBLE_STREAM_MESSAGES_ENV_KEY = "OPENWIKI_OPENAI_COMPATIBLE_STREAM_MESSAGES";
...
export const OPENWIKI_OPENROUTER_PROVIDER_ONLY_ENV_KEY = "OPENWIKI_OPENROUTER_PROVIDER_ONLY";
export const OPENWIKI_OPENROUTER_MAX_TOKENS_ENV_KEY = "OPENWIKI_OPENROUTER_MAX_TOKENS";
...
export const BEDROCK_AWS_SECRET_ACCESS_KEY_ENV_KEY = "BEDROCK_AWS_SECRET_ACCESS_KEY";
...
export const AWS_WEB_IDENTITY_TOKEN_FILE_ENV_KEY = "AWS_WEB_IDENTITY_TOKEN_FILE";
...
export const GOOGLE_APPLICATION_CREDENTIALS_ENV_KEY = "GOOGLE_APPLICATION_CREDENTIALS";
export const DEFAULT_VERTEX_LOCATION = "global";
export const OPENWIKI_PROVIDER_ENV_KEY = "OPENWIKI_PROVIDER";
export const OPENWIKI_MODEL_ID_ENV_KEY = "OPENWIKI_MODEL_ID";
export const OPENWIKI_STREAM_IDLE_TIMEOUT_ENV_KEY = "OPENWIKI_STREAM_IDLE_TIMEOUT";
...
export function providerUsesAwsSdkCredentials(provider) {
    return getProviderAuthMethod(provider) === "aws-sdk";
}
export function providerUsesExternalCliAuth(provider) {
    return getProviderAuthMethod(provider) === "external-cli";
}
export function getProviderExternalCliAuthAdapter(provider) {
    return getProviderConfig(provider).externalCliAuthAdapter;
}
export function providerRequiresApiKey(provider) {
    return (getProviderAuthMethod(provider) === "api-key" &&
        getProviderConfig(provider).apiKeyEnvKey !== undefined);
}
export function providerUsesResponsesApi(provider, modelId) {
    if (provider === "openai-compatible") {
        return resolveOpenAiCompatibleUseResponsesApi();
    }
    const setting = getProviderConfig(provider).responsesApi;
    return (setting === true || (setting instanceof RegExp && setting.test(modelId)));
}
export function providerUsesStreaming(provider) {
    if (provider === "openai-compatible") {
...
export function getProviderProjectEnvKey(provider) {
    return getProviderConfig(provider).projectEnvKey;
}
export function getProviderLocationEnvKey(provider) {
 ...
 */
export function getMissingProviderEnvKey(provider, env = process.env) {
    ...
    if (providerUsesAwsSdkCredentials(provider)) {
        ...
        const legacyPairState = inspectCredentialPair(env, config.apiKeyEnvKey, config.secretKeyEnvKey);
        ...
        const standardPairState = inspectCredentialPair(env, AWS_ACCESS_KEY_ID_ENV_KEY, AWS_SECRET_ACCESS_KEY_ENV_KEY);
 ...
 */
export function resolveProviderLocation(provider, env = process.env) {
    const config = getProviderConfig(provider);
 ...
 */
export function getProviderCredentialHint(provider) {
    if (provider === "gemini-enterprise") {
        return ("Authenticate to Google Cloud with Application Default Credentials " +
            "(gcloud auth application-default login) or set " +
            `${GOOGLE_APPLICATION_CREDENTIALS_ENV_KEY} to a service account key file.`);
    }
    if (providerUsesAwsSdkCredentials(provider)) {
        return ("Configure the AWS SDK credential chain with OIDC/web identity, an IAM " +
...
export function getProviderSecretKeyEnvKey(provider) {
    return getProviderConfig(provider).secretKeyEnvKey;
}
export function providerRequiresSecretKey(provider) {
    return (!providerUsesAwsSdkCredentials(provider) &&
        getProviderConfig(provider).secretKeyEnvKey !== undefined);
}
export function getProviderRegionEnvKey(provider) {
    return getProviderConfig(provider).regionEnvKey;
}
export function getProviderRegionEnvKeys(provider) {
    const config = getProviderConfig(provider);
    return [config.regionEnvKey, ...(config.regionFallbackEnvKeys ?? [])].filter((key) => key !== undefined);
}
export function providerRequiresRegion(provider) {
 ...
 */
export function resolveProviderRegion(provider, env = process.env) {
...
export function getProviderBaseUrlWarnings(provider, value) {
...
export function isValidProviderBaseUrl(provider, value) {
    return getProviderBaseUrlWarnings(provider, value).length === 0;
}
function isChatCompletionsEndpointUrl(value) {
    try {
...
export function getProvidersForKnownModelId(modelId, excludeProvider) {
    ...
    for (const provider of Object.keys(PROVIDER_CONFIGS)) {
        ...
        if (getProviderModelOptions(provider).some((option) => option.id === normalized)) {
...
export function isModelIdForOtherProvider(modelId, provider) {
    const normalized = normalizeModelId(modelId);
    const isKnownForProvider = getProviderModelOptions(provider).some((option) => option.id === normalized);
                            ...
                            : env[FIREWORKS_API_KEY_ENV_KEY]
                                ...
                                : env[NEBIUS_API_KEY_ENV_KEY]
                                    ...
                                    : env[NVIDIA_API_KEY_ENV_KEY]
                                        ? "nvidia"
                                        : hasNonEmptyEnvValue(env, BEDROCK_AWS_ACCESS_KEY_ID_ENV_KEY) ||
                                            hasNonEmptyEnvValue(env, BEDROCK_AWS_SECRET_ACCESS_KEY_ENV_KEY)
                                            ? "bedrock"
                                            : DEFAULT_PROVIDER));
}
...
function inspectCredentialPair(env, accessKeyEnvKey, secretKeyEnvKey) {
...
export function resolveMaxOutputTokens(env = process.env) {
    ...
    if (!/^[1-9]\d*$/u.test(maxOutputTokens)) {
        throw new Error(`Invalid ${OPENWIKI_MAX_OUTPUT_TOKENS_ENV_KEY}. Expected a positive integer.`);
    }
...
export function resolveStreamIdleTimeout(env = process.env) {
    ...
    if (!/^\d+$/u.test(streamIdleTimeout)) {
        throw new Error(`Invalid ${OPENWIKI_STREAM_IDLE_TIMEOUT_ENV_KEY}. Expected an integer from 0 to ${MAX_STREAM_IDLE_TIMEOUT_MS} milliseconds.`);
    }
    const parsedStreamIdleTimeout = Number(streamIdleTimeout);
    if (!Number.isSafeInteger(parsedStreamIdleTimeout) ||
        parsedStreamIdleTimeout > MAX_STREAM_IDLE_TIMEOUT_MS) {
        throw new Error(`Invalid ${OPENWIKI_STREAM_IDLE_TIMEOUT_ENV_KEY}. Expected an integer from 0 to ${MAX_STREAM_IDLE_TIMEOUT_MS} milliseconds.`);
    }
    return parsedStreamIdleTimeout;
}
export function resolveStreamIdleTimeoutForProvider(provider, env = process.env) {
...
export function resolveOpenRouterProviderOnly(env = process.env) {
...
export function resolveOpenAiCompatibleUseResponsesApi(env = process.env) {
    return (env[OPENAI_COMPATIBLE_USE_RESPONSES_API_ENV_KEY]?.trim().toLowerCase() ===
        TRUE_ENV_VALUE);
}
...
export function resolveOpenAiCompatibleStreamMessages(env = process.env) {
    return (env[OPENAI_COMPATIBLE_STREAM_MESSAGES_ENV_KEY]?.trim().toLowerCase() ===
        TRUE_ENV_VALUE);
}
...
export function resolveOpenAiCompatibleStreaming(env = process.env) {
    return (env[OPENAI_COMPATIBLE_STREAMING_ENV_KEY]?.trim().toLowerCase() ===
        TRUE_ENV_VALUE);
}
 ...
 */
export function resolveOpenRouterMaxTokens(env = process.env) {
    return resolvePositiveIntegerSetting(OPENWIKI_OPENROUTER_MAX_TOKENS_ENV_KEY, env);
}
/**
 ...
 * @returns The configured positive integer, or `undefined` when unset.
 */
export function resolveConfiguredMaxOutputTokens(provider, env = process.env) {
 ...
 */
function resolvePositiveIntegerSetting(envKey, env) {
...
export function resolveBedrockMaxTokens(env = process.env) {
    ...
    const maxTokens = rawMaxTokens.trim();
    if (!/^[1-9]\d*$/u.test(maxTokens)) {
        throw new Error(`Invalid ${OPENWIKI_BEDROCK_MAX_TOKENS_ENV_KEY}. Expected a positive integer.`);
    }
    const parsedMaxTokens = Number(maxTokens);
    if (!Number.isSafeInteger(parsedMaxTokens)) {
        throw new Error(`Invalid ${OPENWIKI_BEDROCK_MAX_TOKENS_ENV_KEY}. Expected a positive integer.`);
