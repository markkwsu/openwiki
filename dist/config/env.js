...
import { ANTHROPIC_API_KEY_ENV_KEY, ANTHROPIC_BASE_URL_ENV_KEY, BASETEN_API_KEY_ENV_KEY, BASETEN_BASE_URL_ENV_KEY, BEDROCK_AWS_ACCESS_KEY_ID_ENV_KEY, BEDROCK_AWS_REGION_ENV_KEY, BEDROCK_AWS_SECRET_ACCESS_KEY_ENV_KEY, OPENWIKI_BEDROCK_MAX_TOKENS_ENV_KEY, COPILOT_API_KEY_ENV_KEY, COPILOT_BASE_URL_ENV_KEY, FIREWORKS_API_KEY_ENV_KEY, FIREWORKS_BASE_URL_ENV_KEY, getProviderBaseUrlWarnings, GEMINI_API_KEY_ENV_KEY, GOOGLE_APPLICATION_CREDENTIALS_ENV_KEY, GOOGLE_CLOUD_LOCATION_ENV_KEY, GOOGLE_CLOUD_PROJECT_ENV_KEY, isValidModelId, NEBIUS_API_KEY_ENV_KEY, normalizeProvider, NVIDIA_API_KEY_ENV_KEY, NVIDIA_BASE_URL_ENV_KEY, OPENAI_API_KEY_ENV_KEY, OPENAI_BASE_URL_ENV_KEY, OPENAI_CHATGPT_ACCESS_TOKEN_ENV_KEY, OPENAI_CHATGPT_ACCOUNT_ID_ENV_KEY, OPENAI_CHATGPT_EMAIL_ENV_KEY, OPENAI_CHATGPT_EXPIRES_AT_ENV_KEY, OPENAI_CHATGPT_PLAN_ENV_KEY, OPENAI_CHATGPT_REFRESH_TOKEN_ENV_KEY, OPENAI_COMPATIBLE_API_KEY_ENV_KEY, OPENAI_COMPATIBLE_BASE_URL_ENV_KEY, OPENAI_COMPATIBLE_STREAMING_ENV_KEY, OPENAI_COMPATIBLE_USE_RESPONSES_API_ENV_KEY, OPENWIKI_GOOGLE_ACCESS_TOKEN_ENV_KEY, OPENWIKI_GOOGLE_CLIENT_ID_ENV_KEY, OPENWIKI_GOOGLE_CLIENT_SECRET_ENV_KEY, OPENWIKI_GOOGLE_REFRESH_TOKEN_ENV_KEY, OPENWIKI_GMAIL_ACCESS_TOKEN_ENV_KEY, OPENWIKI_GMAIL_REFRESH_TOKEN_ENV_KEY, OPENWIKI_NOTION_MCP_ACCESS_TOKEN_ENV_KEY, OPENWIKI_NOTION_MCP_CLIENT_ID_ENV_KEY, OPENWIKI_NOTION_MCP_REFRESH_TOKEN_ENV_KEY, OPENROUTER_API_KEY_ENV_KEY, OPENWIKI_NOTION_TOKEN_ENV_KEY, OPENWIKI_OPENROUTER_MAX_TOKENS_ENV_KEY, OPENWIKI_OPENROUTER_PROVIDER_ONLY_ENV_KEY, OPENWIKI_SLACK_BOT_TOKEN_ENV_KEY, OPENWIKI_SLACK_CLIENT_ID_ENV_KEY, OPENWIKI_SLACK_CLIENT_SECRET_ENV_KEY, OPENWIKI_SLACK_USER_TOKEN_ENV_KEY, OPENWIKI_X_ACCESS_TOKEN_ENV_KEY, OPENWIKI_X_CLIENT_ID_ENV_KEY, OPENWIKI_X_CLIENT_SECRET_ENV_KEY, OPENWIKI_X_REFRESH_TOKEN_ENV_KEY, OPENWIKI_TAVILY_API_KEY_ENV_KEY, OPENWIKI_MAX_OUTPUT_TOKENS_ENV_KEY, OPENWIKI_MODEL_ID_ENV_KEY, OPENWIKI_PROVIDER_ENV_KEY, OPENWIKI_REASONING_EFFORT_ENV_KEY, OPENWIKI_PROVIDER_RETRY_ATTEMPTS_ENV_KEY, OPENWIKI_STREAM_IDLE_TIMEOUT_ENV_KEY, resolveConfiguredProvider, resolveBedrockMaxTokens, resolveMaxOutputTokens, resolveOpenRouterMaxTokens, resolveProviderRetryAttempts, resolveStreamIdleTimeout, } from "./constants.js";
 ...
 * `process.env`.
 */
export function getShellEnvValue(key) {
 ...
 */
export function getSavedEnvValue(key) {
...
export async function getCredentialDiagnostics() {
    ...
    return CREDENTIAL_DIAGNOSTIC_ENV_KEYS.map((key) => createCredentialDiagnostic(key, fileEnv, provider));
}
let saveOpenWikiEnvQueue = Promise.resolve();
export function saveOpenWikiEnv(updates) {
    ...
    saveOpenWikiEnvQueue = save.then(() => undefined, () => undefined);
    return save;
}
async function saveOpenWikiEnvLocked(updates) {
    captureShellEnv();
...
function createCredentialDiagnostic(key, fileEnv, provider) {
...
function getBaseUrlDiagnosticWarnings(key, value) {
...
function getBooleanWarnings(value) {
...
function getMaxOutputTokensWarnings(value) {
    try {
...
function getBedrockMaxTokensWarnings(value) {
    try {
...
function getOpenRouterMaxTokensWarnings(value) {
    try {
...
function getStreamIdleTimeoutWarnings(value, provider) {
    if (provider !== "bedrock") {
        return [];
    }
    try {
...
function getReasoningEffortWarnings(value) {
...
function parseEnvValue(value) {
    if (value.startsWith('"') && value.endsWith('"')) {
        ...
        return value.slice(1, -1).replace(/\\(.)/gsu, (match, escaped) => {
            switch (escaped) {
                case "n":
                    return "\n";
                case "r":
