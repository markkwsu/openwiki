 ...
 */
async function collectPaginatedTools(request) {
    const tools = [];
...
class StdioJsonRpcClient {
    ...
    listTools() {
        return collectPaginatedTools((method, params) => this.request(method, params));
    }
    close() {
...
class HttpJsonRpcClient {
    ...
    async post(message) {
        const response = await fetchWithResilience(this.url, {
...
export function buildChildEnv(envRefs) {
    const base = {};
