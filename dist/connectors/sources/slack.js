...
const DEFAULT_CONVERSATION_TYPES = [
    "public_channel",
    "private_channel",
    "im",
    "mpim",
];
const VALID_SLACK_STREAMS = [
    "assistant_search",
    "my_messages_search",
    "recent_messages",
];
const VALID_SLACK_CONVERSATION_TYPES = [
    "im",
    "mpim",
    "private_channel",
    "public_channel",
...
const definition = {
    backend: "direct-api",
    description: "Fetches Slack conversations, recent messages, and assistant search context with a Slack user token.",
    displayName: "Slack",
    id: "slack",
    mode: "personal",
...
async function ingest(options = {}) {
    ...
    const config = {
        ...(await readConnectorConfig("slack", {
    ...
    if (!config.enabled) {
        return {
            connectorId: "slack",
            message: "Slack connector is not enabled. Run openwiki auth configure slack --force to generate the direct Slack API config.",
            rawFiles,
            runId,
    ...
    let myMessagesSearchError;
    rawFiles.push(await writeRawJson("slack", runId, "identity.json", {
    ...
    if (streams.includes("my_messages_search")) {
        if (userId) {
            try {
                myMessagesSearch = await fetchMyMessagesSearch(accessToken, userId, config);
                rawFiles.push(await writeRawJson("slack", runId, "my-messages-search.json", {
                ...
                warnings.push(`Slack self-message search failed; falling back to bounded conversation history. ${myMessagesSearchError}`);
            }
        }
        else {
            warnings.push("Slack self-message search was skipped because auth.test did not return a user_id.");
        }
    }
    if (streams.includes("recent_messages")) {
        ...
        if (recent.userMessages.length <= 1) {
            warnings.push("Slack found one or fewer authenticated-user messages in the bounded recent history window; latestMessage may not be the user's true latest Slack message.");
        }
        rawFiles.push(await writeRawJson("slack", runId, "recent-messages.json", {
    ...
    if (myMessagesSearch || recent) {
        const latestSource = myMessagesSearch && myMessagesSearch.userMessages.length > 0
            ? "search.messages"
            : "conversations.history";
        const latestMessages = latestSource === "search.messages"
            ? (myMessagesSearch?.userMessages ?? [])
            : (recent?.userMessages ?? []);
        rawFiles.push(await writeRawJson("slack", runId, "my-recent-messages.json", {
            coverage: {
            ...
            note: latestSource === "search.messages"
    ...
    if (streams.includes("assistant_search")) {
        ...
        if (searches.length > 0) {
            rawFiles.push(await writeRawJson("slack", runId, "assistant-search.json", {
                fetchedAt: new Date().toISOString(),
                searches,
                userId,
            }));
            ...
            warnings.push("assistant_search requested but assistantSearchQueries is empty.");
        }
    }
    await writeConnectorState("slack", updateStateWithRun(state, {
...
async function fetchRecentMessages(accessToken, config, userId) {
...
async function fetchMyMessagesSearch(accessToken, userId, config) {
...
async function fetchSlackUser(accessToken, userId) {
    try {
...
async function fetchConversations(accessToken, config) {
    ...
    while (conversations.length < conversationScanLimit) {
        const response = await slackApi(accessToken, "conversations.list", {
            ...
            limit: String(Math.min(200, conversationScanLimit - conversations.length)),
            types,
        });
        conversations.push(...(response.channels ?? []).filter((conversation) => conversation.id && !conversation.is_archived));
        cursor = response.response_metadata?.next_cursor;
        if (!cursor) {
            break;
        }
    }
    const sortedConversations = dedupeConversations(conversations).sort(compareSlackConversationsByUpdated);
...
async function slackApi(accessToken, method, params) {
    const response = await fetchWithResilience(`${SLACK_API_BASE_URL}/${method}`, {
...
function normalizeStreams(optionStreams, configStreams) {
    ...
    if (normalized.includes("recent_messages") &&
        !normalized.includes("my_messages_search")) {
        return ["my_messages_search", ...normalized];
    }
    return normalized;
}
function normalizeConversationTypes(configTypes) {
    const types = Array.isArray(configTypes) && configTypes.length > 0
        ? configTypes
        : DEFAULT_CONVERSATION_TYPES;
    return types.filter(isSlackConversationType);
}
function isSlackStream(value) {
    return typeof value === "string" && VALID_SLACK_STREAMS.includes(value);
}
function isSlackConversationType(value) {
    return (typeof value === "string" && VALID_SLACK_CONVERSATION_TYPES.includes(value));
}
...
function getRecentMessageCoverage(config, scan) {
    return {
...
function compareSlackUserMessages(left, right) {
    return (slackTimestampToNumber(right.message.ts) -
        slackTimestampToNumber(left.message.ts));
}
function slackTimestampToNumber(value) {
    if (!value) {
        return 0;
    }
...
function sanitizeSlackUserId(userId) {
    if (!/^[UW][A-Z0-9]+$/u.test(userId)) {
        throw new Error("Slack auth.test returned an invalid user_id.");
    }
    return userId;
}
function compareSlackConversationsByUpdated(left, right) {
    return (slackConversationUpdatedToNumber(right) -
        slackConversationUpdatedToNumber(left));
}
function slackConversationUpdatedToNumber(conversation) {
