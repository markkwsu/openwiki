 ...
 * most-recent-first.
 */
export function selectSampleBuckets(errorRuns, nonErrorRuns, caps) {
    const selected = [];
    const usedIds = new Set();
    const take = (run, bucket) => {
    ...
    const outlierBudget = Math.min(caps.outlierCap, caps.total - selected.length, Math.floor(nonErrorRuns.length / 4));
 ...
 */
export function compactTrace(runs, projectUrl, bucket, maxFieldChars) {
    if (runs.length === 0) {
        return undefined;
    }
 ...
 */
export function summarizeSample(selected) {
    const bucketCounts = {
        baseline: 0,
        error: 0,
        outlier: 0,
 ...
 */
function compactTraceRun(run, maxFieldChars) {
    return {
        id: run.id,
 ...
 */
export function isErrorRun(run) {
