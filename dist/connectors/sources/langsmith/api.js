 ...
 */
export function createLangSmithApi(baseUrl, apiKey) {
    ...
    return {
        ...
        async listRootRuns(projectId, { limit, startTime }) {
            return withRateLimitRetry(() => drainCapped(client.listRuns({
                isRoot: true,
                limit,
                order: "desc",
                projectId,
        ...
        async fetchTrace(traceId) {
            return withRateLimitRetry(() => drainCapped(client.listRuns({ select: RUN_SELECT_FIELDS, traceId }), MAX_TRACE_RUNS));
        },
    };
}
/**
 ...
 */
async function drainCapped(stream, cap) {
    const items = [];
