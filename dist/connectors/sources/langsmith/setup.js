import { readLangSmithRepoConfig, writeLangSmithRepoConfig, } from "./repo-config.js";
/**
 ...
 */
export async function loadLangSmithSetup(repoRoot) {
    ...
    return (config?.workspaces ?? []).map((workspace) => ({
        ...
        region: workspace.apiBaseUrl === EU_API_BASE_URL
            ? "eu"
 ...
 */
export async function saveLangSmithSetup(repoRoot, workspaces) {
 ...
 */
export function nextLangSmithApiKeyEnv(existing) {
