...
import { createRunId, readConnectorState, updateStateWithRun, writeConnectorState, writeRawJson, } from "../../io.js";
import { createLangSmithApi } from "./api.js";
import { readLangSmithRepoConfig, sanitizeLangSmithApiBaseUrl, sanitizeLangSmithApiKeyEnv, } from "./repo-config.js";
import { compactTrace, isErrorRun, selectSampleBuckets, summarizeSample, } from "./runs.js";
/**
...
const definition = {
    backend: "direct-api",
    description: "Pulls recent LangSmith traces (tool calls, outcomes, latency) through the official LangSmith SDK.",
 ...
 */
async function loadWorkspaces(repoRoot) {
...
async function ingest(options = {}) {
    ...
    if (workspaces.length === 0) {
        return result(runId, [], [], "skipped", "LangSmith is not configured for this repository.");
    }
    ...
    const windowStart = options.windowHours !== undefined
    ...
    for (const workspace of workspaces) {
        ...
        const apiBaseUrl = sanitizeLangSmithApiBaseUrl(workspace.apiBaseUrl) ?? DEFAULT_API_BASE_URL;
        const apiKeyEnv = sanitizeLangSmithApiKeyEnv(workspace.apiKeyEnv);
        const projects = normalizeProjects(workspace.projects);
        projectCount += projects.length;
        if (!apiKeyEnv) {
            warnings.push(`${sanitizeDiagnosticText(String(workspace.apiKeyEnv))}: not an allowed LangSmith key env var.`);
            continue;
        }
        ...
        for (const project of projects) {
            try {
                const pull = await pullProject(api, project, apiBaseUrl, windowStart, bounds, warnings);
                if (pull) {
                    pulls.push(pull);
                }
            }
                ...
                warnings.push(`${project.name}: ${sanitizeDiagnosticText(errorMessage(err))}`);
            }
        }
    }
    const rawFiles = pulls.length > 0
        ? [
    ...
    return result(runId, warnings, rawFiles, status, `Pulled ${pulls.length} of ${projectCount} LangSmith project(s).`);
}
/**
 ...
 */
async function pullProject(api, project, apiBaseUrl, windowStart, bounds, warnings) {
    const { id: projectId, url: projectUrl } = await api.resolveProject(project.name);
    ...
    const selected = selectSampleBuckets(recentRoots.filter(isErrorRun), recentRoots.filter((run) => !isErrorRun(run)), { errorCap: ERROR_CAP, outlierCap: OUTLIER_CAP, total: MAX_TRACES });
    ...
    for (const { bucket, run } of selected) {
        try {
            ...
            const trace = compactTrace(runs, projectUrl, bucket, bounds.maxFieldChars);
            if (trace) {
                traces.push(trace);
            }
        }
            ...
            warnings.push(`${project.name}: skipped a trace: ${sanitizeDiagnosticText(errorMessage(err))}`);
        }
    }
    if (traces.length === 0) {
        return undefined;
 ...
 */
function result(runId, warnings, rawFiles, status, message) {
    return {
