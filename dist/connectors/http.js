 ...
 */
export function parseRetryAfterMs(headerValue, now) {
    if (!headerValue) {
        return null;
    }
...
function backoffDelayMs(attempt, baseDelayMs, random) {
 ...
 */
export async function fetchWithResilience(input, init = {}, options = {}) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt += 1) {
        ...
        try {
            ...
            if (attempt < maxRetries && isRetryableStatus(response.status)) {
                ...
                const delay = Math.min(MAX_BACKOFF_DELAY_MS, retryAfterMs ?? backoffDelayMs(attempt, baseDelayMs, random));
