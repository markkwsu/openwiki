...
export function createSystemPrompt(command, outputMode = "local-wiki", language, openWikiIgnore) {
    if (outputMode === "repository" && command !== "chat") {
        throw new Error("Repository generation does not use shared agent prompts.");
    }
    const template = outputMode === "repository"
        ? CODE_SYSTEM_PROMPTS.chat
        : PERSONAL_SYSTEM_PROMPTS[command];
    const prompt = template
        .replace("{OUTPUT_LANGUAGE_INSTRUCTIONS}", formatLanguageInstructions(language))
        .replace("{GIT_HISTORY_HINT}", formatGitHistoryHint(openWikiIgnore))
        .replace("{DISCOVERY_INSTRUCTION}", formatDiscoveryInstruction(openWikiIgnore))
        .replace("{OPENWIKIIGNORE_INSTRUCTIONS}", formatOpenWikiIgnoreInstructions(openWikiIgnore))
        .trim();
    return command === "chat"
        ? prompt
 ...
 */
export function createUserPrompt(command, context, userMessage = null, outputMode = "local-wiki", runtimeRoot) {
    if (outputMode === "repository" && command !== "chat") {
        throw new Error("Repository generation does not use shared agent prompts.");
    }
    const template = outputMode === "repository"
    ...
    return template
        ...
        .replace("{ADDITIONAL_USER_REQUEST}", userMessage?.trim()
        ? `Additional user instruction:\n${userMessage.trim()}`
        : "")
        .replace("{RUNTIME_CONTEXT}", runtimeRoot ? formatRuntimeContext(runtimeRoot, outputMode) : "")
        .trim();
}
export function formatRuntimeRootInstruction(outputMode) {
...
function formatLanguageInstructions(language) {
    if (!language) {
        return "";
    }
    return `
...
export function createLinkIntegrityInstructions() {
    return `
Link integrity:
...
export function createDiagramInstructions() {
    return `
Diagram discipline:
...
function formatGitHistoryHint(openWikiIgnore) {
...
function formatDiscoveryInstruction(openWikiIgnore) {
    return openWikiIgnore?.isActive
...
function formatOpenWikiIgnoreInstructions(openWikiIgnore) {
...
function formatRuntimeContext(runtimeRoot, outputMode) {
    const rootLabel = outputMode === "local-wiki" ? "Local wiki root" : "Repository root";
    return `${rootLabel}:
${runtimeRoot}
