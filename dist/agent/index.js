...
import { DEBUG_ENV_KEYS, loadOpenWikiEnv, openWikiEnvDir, saveOpenWikiEnv, } from "../config/env.js";
import { isFileNotFoundError } from "../platform/fs-errors.js";
import { sanitizeDiagnosticText, SECRET_KEY_PATTERN_SOURCE, } from "../platform/diagnostics.js";
import { openWikiHomeDisplayPath, openWikiLocalWikiDir, } from "../config/openwiki-home.js";
import { requireResolvedLanguage } from "../platform/language.js";
import { resolveConceptTypeLabel, resolveIndexLabels, } from "../okf/index-labels.js";
...
import { createWikiTranslationMiddleware, resolveTranslationPlan, } from "./translation-middleware.js";
import { CODEX_ORIGINATOR, CODEX_RESPONSES_BASE_URL, codexTokensToEnv, createCodexFetch, isChatGptTokenExpired, readCodexTokensFromEnv, refreshChatGptTokens, } from "./openai-chatgpt-oauth.js";
...
import { createVertexAuthFetch, resolveVertexSurface, stripPublisherPath, toVertexPublisherModel, vertexOpenAIBaseUrl, withAnthropicAuthEnvNeutralized, } from "./vertex-surface.js";
import { ANTHROPIC_BASE_URL_ENV_KEY, BASETEN_BASE_URL_ENV_KEY, BEDROCK_AWS_ACCESS_KEY_ID_ENV_KEY, BEDROCK_AWS_REGION_ENV_KEY, BEDROCK_AWS_SECRET_ACCESS_KEY_ENV_KEY, BEDROCK_AWS_SESSION_TOKEN_ENV_KEY, COPILOT_BASE_URL_ENV_KEY, DEFAULT_ANTHROPIC_MAX_OUTPUT_TOKENS, getDefaultModelId, getMissingProviderEnvKey, getProviderApiKeyEnvKey, getProviderBaseUrlEnvKey, getProviderCredentialHint, getProviderLabel, getProviderBaseUrlWarnings, getProviderModelOptions, FIREWORKS_BASE_URL_ENV_KEY, getProviderRegionEnvKeys, getProviderSecretKeyEnvKey, getProvidersForKnownModelId, isModelIdForOtherProvider, DEFAULT_VERTEX_LOCATION, GOOGLE_CLOUD_PROJECT_ENV_KEY, isValidModelId, normalizeModelId, NVIDIA_BASE_URL_ENV_KEY, OPENAI_BASE_URL_ENV_KEY, OPENAI_COMPATIBLE_BASE_URL_ENV_KEY, OPENAI_COMPATIBLE_STREAMING_ENV_KEY, OPENROUTER_API_KEY_ENV_KEY, OPENROUTER_BASE_URL, OPENWIKI_MAX_OUTPUT_TOKENS_ENV_KEY, OPENWIKI_MODEL_ID_ENV_KEY, OPENWIKI_OPENROUTER_MAX_TOKENS_ENV_KEY, OPENWIKI_PROVIDER_ENV_KEY, OPENWIKI_PROVIDER_RETRY_ATTEMPTS_ENV_KEY, OPENWIKI_STREAM_IDLE_TIMEOUT_ENV_KEY, providerRequiresBaseUrl, providerRequiresRegion, providerRequiresSecretKey, providerUsesAwsSdkCredentials, providerUsesExternalCliAuth, providerUsesResponsesApi, providerUsesStreaming, resolveConfiguredMaxOutputTokens, resolveConfiguredProvider, resolveOpenAiCompatibleStreamMessages, resolveOpenRouterMaxTokens, resolveOpenRouterProviderOnly, resolveProviderBaseUrl, resolveProviderLocation, resolveProviderRegion, resolveProviderRetryAttempts, resolveStreamIdleTimeoutForProvider, } from "../config/constants.js";
import { resolveReasoningConfig } from "../config/reasoning.js";
import { resolveExternalCliCredential, validateExternalCliCredential, } from "../auth/external-cli-auth.js";
import { createOpenWikiContentSnapshot, createRunContext, persistRunMetadataIfChanged, } from "./utils.js";
...
export async function runOpenWikiAgent(command, cwd = openWikiLocalWikiDir, options = {}, telemetryContext = {}) {
    ...
    const isRepositoryGeneration = outputMode === "repository" && (command === "init" || command === "update");
    if (isRepositoryGeneration) {
        ...
        try {
            ...
            const model = inStageSync("build", () => createModel(config.provider, config.modelId, config.providerRetryAttempts, config.maxOutputTokens, config.streamIdleTimeout), { errorClass: "build_error", errorDetail: "model" });
            const generation = await inStage("run", () => runNativeRepositoryGeneration({
                root: runtimeCwd,
                mode: command,
    ...
    const openWikiIgnore = outputMode === "repository"
        ? await OpenWikiIgnore.load(runtimeCwd)
        : new OpenWikiIgnore([]);
    emitDebug(options, `openwikiignore.patterns=${openWikiIgnore.patterns.length}`);
    const debugFetchCapture = installOpenRouterDebugFetch(options);
    try {
        ...
        return await runOpenWikiAgentCore(command, runtimeCwd, options, config.provider, config.modelId, config.providerRetryAttempts, config.maxOutputTokens, config.streamIdleTimeout, openWikiIgnore, runTimestamp);
    }
    catch (error) {
 ...
 */
async function resolveRunConfig(options, onProviderResolved) {
    try {
        ...
        if (providerBaseUrl) {
            emitDebug(options, `provider.baseUrl=${formatUrlDebugValue(providerBaseUrl)}`);
        }
        ...
        emitDebug(options, providerUsesAwsSdkCredentials(provider)
        ...
        if (modelAvailability.status === "unavailable") {
            throw new Error(`${getProviderLabel(provider)} does not make model "${modelId}" available to the configured credentials. Set ${OPENWIKI_MODEL_ID_ENV_KEY} to an available model.`);
        }
        if (modelAvailability.status === "unknown") {
            emitDebug(options, `model.availability=unknown${modelAvailability.reason ? ` reason=${modelAvailability.reason}` : ""}`);
        }
        ...
        emitDebug(options, `model.maxOutputTokens=${maxOutputTokens ?? "provider-default"}`);
        const streamIdleTimeout = resolveStreamIdleTimeoutForProvider(provider);
        emitDebug(options, `model.streamIdleTimeout=${streamIdleTimeout ?? "provider-default"}`);
        return {
            provider,
            modelId,
 ...
 */
export async function createOpenWikiAgent(options) {
    if (!path.isAbsolute(options.cwd)) {
        throw new Error("OpenWiki agent cwd must be an absolute path.");
    }
    if (options.outputMode === "repository" && options.command !== "chat") {
        throw new Error("Repository init/update use the OpenWiki page-job runner; call runOpenWikiAgent instead of createOpenWikiAgent.");
    }
    await syncBundledSkills();
    const openWikiIgnore = options.outputMode === "repository"
        ? await OpenWikiIgnore.load(options.cwd)
        : new OpenWikiIgnore([]);
    const context = await createRunContext(options.cwd, options.outputMode, options.language);
    const checkpointer = await createCheckpointer(resolveCheckpointTarget(options.command));
...
function createOpenWikiAgentGraph(options) {
    ...
    const translation = resolveTranslationPlan(options.command, requireResolvedLanguage(options.language), options.context.lastUpdate?.language);
    ...
    return createDeepAgent({
        ...
        backend,
        middleware: options.command === "chat"
            ? []
            : [
                ...(translation
                    ? [
                        createWikiTranslationMiddleware(wikiBackend, options.outputMode, options.model, translation, (message) => {
                    ...
                    : []),
                createOpenWikiIndexMiddleware(wikiBackend, options.outputMode, indexLabels, conceptType, options.runTimestamp),
            ],
        skills: ["/skills/"],
        subagents: [],
        permissions: AGENT_FILESYSTEM_PERMISSIONS,
        systemPrompt: createSystemPrompt(options.command, options.outputMode, options.context.language, options.openWikiIgnore),
    });
}
async function runOpenWikiAgentCore(command, cwd, options, provider, modelId, providerRetryAttempts, maxOutputTokens, streamIdleTimeout, openWikiIgnore, runTimestamp) {
    const outputMode = options.outputMode ?? "local-wiki";
    const context = await inStage("build", () => createRunContext(cwd, outputMode, options.language), { errorClass: "build_error", errorDetail: "run_context" });
    emitDebug(options, "context=created");
    const openWikiSnapshotBefore = command === "chat"
        ? null
        : await inStage("build", () => createOpenWikiContentSnapshot(cwd, outputMode), { errorClass: "build_error", errorDetail: "snapshot" });
    emitDebug(options, "openwiki.snapshot=created");
    const model = inStageSync("build", () => createModel(provider, modelId, providerRetryAttempts, maxOutputTokens, streamIdleTimeout), { errorClass: "build_error", errorDetail: "model" });
    ...
    const checkpointer = await inStage("build", () => createCheckpointer(checkpointTarget), { errorClass: "checkpointer_error", errorDetail: "create" });
    emitDebug(options, checkpointTarget.persistent
        ? `checkpointer=${formatUrlDebugValue(checkpointTarget.connString)}`
        : "checkpointer=memory");
    const agent = inStageSync("build", () => createOpenWikiAgentGraph({
        command,
        cwd,
        language: options.language,
        model,
    ...
    const streamMessagesEnabled = provider !== "openai-compatible" || resolveOpenAiCompatibleStreamMessages();
    ...
    const stream = await inStage("build", () => agent.stream(input, {
        configurable: {
            thread_id: threadId,
        },
    ...
    try {
        ...
        try {
            const metadataWritten = await persistRunMetadataIfChanged(command, cwd, modelId, outputMode, openWikiSnapshotBefore, "interrupted", context.language);
            emitDebug(options, metadataWritten ? "metadata=written" : "metadata=skipped");
        }
        catch {
        ...
        clearActiveRun();
        prunePersistentCheckpointHistory(checkpointTarget, checkpointer, threadId, options);
    }
    if (checkpointTarget.persistent) {
        ...
        await inStage("finalize", () => chmodIfExists(checkpointTarget.connString, 0o600), { errorClass: "checkpointer_error", errorDetail: "chmod" });
    }
    ...
    try {
        metadataWritten = await inStage("finalize", async () => {
            return persistRunMetadataIfChanged(command, cwd, modelId, outputMode, openWikiSnapshotBefore, "complete", context.language);
        });
    }
    catch (error) {
        try {
            await persistRunMetadataIfChanged(command, cwd, modelId, outputMode, openWikiSnapshotBefore, "interrupted", context.language);
        }
        catch {
            emitDebug(options, "metadata=writeFailed");
        }
...
function createRunUserMessage(command, cwd, context, options) {
    ...
    return createUserPrompt(command, context, options.userMessage ?? null, options.outputMode ?? "local-wiki", cwd);
}
const checkpointPath = path.join(openWikiEnvDir, "openwiki.sqlite");
async function createCheckpointer(target) {
    if (target.persistent) {
        await prepareCheckpointDirectory(target.connString);
    }
    return SqliteSaver.fromConnString(target.connString);
}
async function prepareCheckpointDirectory(filePath) {
...
export function pruneCheckpointHistory(checkpointer, threadId) {
    const prune = checkpointer.db.transaction((id) => {
        checkpointer.db
            .prepare(`DELETE FROM checkpoints
         WHERE thread_id = ?
...
function prunePersistentCheckpointHistory(checkpointTarget, checkpointer, threadId, options) {
    if (!checkpointTarget.persistent) {
        return;
    }
    try {
...
export function resolveCheckpointTarget(command) {
    if (command === "chat") {
        return {
...
function ensureProviderCredentials(provider) {
    ...
    throw new Error(`${missingEnvKey} is required to run OpenWiki with ${getProviderLabel(provider)}.${hint ? ` ${hint}` : ""}`);
}
...
function ensureProviderSecretKey(provider) {
    ...
    if (secretKeyEnvKey && !process.env[secretKeyEnvKey]) {
        throw new Error(`${secretKeyEnvKey} is required to run OpenWiki with ${getProviderLabel(provider)}.`);
    }
}
function ensureProviderRegion(provider) {
    if (!providerRequiresRegion(provider)) {
        return;
    }
    if (!resolveProviderRegion(provider)) {
        const regionEnvKeys = getProviderRegionEnvKeys(provider);
        const regionRequirement = regionEnvKeys.length > 0 ? regionEnvKeys.join(", ") : "region";
        throw new Error(`One of ${regionRequirement} is required to run OpenWiki with ${getProviderLabel(provider)}.`);
    }
}
export function resolveModelId(options, provider) {
    const configuredModelId = options.modelId ?? process.env[OPENWIKI_MODEL_ID_ENV_KEY];
    if (!configuredModelId && getProviderModelOptions(provider).length === 0) {
        throw new Error(`${OPENWIKI_MODEL_ID_ENV_KEY} is required to run OpenWiki with ${getProviderLabel(provider)}.`);
    }
    const modelId = normalizeModelId(configuredModelId ?? getDefaultModelId(provider));
...
function warnOnProviderModelMismatch(options, provider, modelId) {
    ...
    const otherProviders = getProvidersForKnownModelId(modelId, provider)
        ...
        .join(", ");
    const message = `Warning: model "${modelId}" is not a known ${getProviderLabel(provider)} model ` +
...
export function createModel(provider, modelId, providerRetryAttempts, maxOutputTokens, streamIdleTimeout) {
    const retryOptions = { maxRetries: providerRetryAttempts };
    const configuredMaxOutputTokens = maxOutputTokens ?? resolveConfiguredMaxOutputTokens(provider);
    const maxTokensOptions = configuredMaxOutputTokens === undefined
        ? {}
        : { maxTokens: configuredMaxOutputTokens };
    const googleMaxOutputTokensOptions = configuredMaxOutputTokens === undefined
        ? {}
        : { maxOutputTokens: configuredMaxOutputTokens };
    const streamIdleTimeoutOptions = streamIdleTimeout === undefined ? {} : { streamIdleTimeout };
    ...
    const responsesReasoningOptions = reasoningConfig?.transport === "responses-reasoning"
        ? { reasoning: { effort: reasoningConfig.effort } }
        : {};
    const chatCompletionsReasoningOptions = reasoningConfig?.transport === "chat-completions-reasoning-effort"
    ...
    if (provider === "gemini-enterprise") {
        ...
        if (!projectId) {
            throw new Error(`${GOOGLE_CLOUD_PROJECT_ENV_KEY} is required for the gemini-enterprise provider.`);
        }
        ...
        const location = resolveProviderLocation(provider) ?? DEFAULT_VERTEX_LOCATION;
        return createGeminiEnterpriseModel(modelId, projectId, location, retryOptions, configuredMaxOutputTokens);
    }
    if (provider === "anthropic") {
        const baseURL = resolveProviderBaseUrl(provider);
        const maxTokens = resolveAnthropicMaxOutputTokens(modelId, configuredMaxOutputTokens);
...
const CHATGPT_LOGIN_INCOMPLETE_MESSAGE = "ChatGPT login is incomplete. Run `openwiki code --init` or `openwiki personal --init` to sign in with your ChatGPT account.";
/**
 ...
 */
async function ensureFreshChatGptTokens() {
    const tokens = readCodexTokensFromEnv();
    if (!tokens) {
        throw new Error(CHATGPT_LOGIN_INCOMPLETE_MESSAGE);
    }
    ...
    await saveOpenWikiEnv(codexTokensToEnv(await refreshChatGptTokens(tokens.refresh)));
}
 ...
 */
function resolveAnthropicMaxOutputTokens(modelId, configuredMaxOutputTokens) {
    if (configuredMaxOutputTokens !== undefined) {
        return configuredMaxOutputTokens;
    }
    const normalizedModelId = stripPublisherPath(modelId);
    return /^claude-(?:haiku|sonnet|opus)-(?:4|5)(?:[-.@]|$)/u.test(normalizedModelId)
 ...
 */
function createGeminiEnterpriseModel(modelId, projectId, location, retryOptions, maxOutputTokens) {
    const maxTokensOptions = maxOutputTokens === undefined ? {} : { maxTokens: maxOutputTokens };
    const googleMaxOutputTokensOptions = maxOutputTokens === undefined ? {} : { maxOutputTokens };
    switch (resolveVertexSurface(modelId)) {
        case "anthropic": {
            const maxTokens = resolveAnthropicMaxOutputTokens(modelId, maxOutputTokens);
            ...
            return new ChatAnthropic(stripPublisherPath(modelId), {
                createClient: () => withAnthropicAuthEnvNeutralized(() => new AnthropicVertex({
                    projectId,
                    region: location,
...
export function parseAgentStreamChunk(chunk) {
 ...
 */
export function parseStreamEvent(chunk) {
...
function isProtocolStreamEvent(value) {
 ...
 */
function parseUpdatesChunk(namespace, payload) {
    if (!isRecord(payload)) {
        return null;
    }
...
function parseToolStreamEvent(payload) {
    ...
    if (event === "on_tool_start") {
        return {
            type: "tool_start",
            call: sanitizeDiagnosticText(`${formatToolCallName(name)}(${formatToolArgs(payload.input)})`),
            id,
            input: payload.input,
            name,
        };
 ...
 */
function getStreamSource(namespace) {
...
const activeOpenRouterSinks = new Set();
let openRouterOriginalFetch = null;
function openRouterDebugFetch(input, init) {
    ...
    return (async () => {
        try {
            ...
            if (!response.ok) {
                ...
                for (const sink of activeOpenRouterSinks) {
                    emitDebug(sink.options, `openrouter.http status=${response.status} statusText=${JSON.stringify(response.statusText)}`);
                }
            }
            return response;
        }
 ...
 */
export function installOpenRouterDebugFetch(options) {
    const sink = { lastFailure: null, options };
...
export function sanitizeOpenRouterResponseBody(body) {
    ...
    const secretJsonKeyPattern = new RegExp(`"([^"]*(?:${SECRET_KEY_PATTERN_SOURCE})[^"]*)"\\s*:\\s*"[^"]*"`, "giu");
    return body.replace(secretJsonKeyPattern, (_, key) => `${JSON.stringify(key)}:"[REDACTED]"`);
}
...
function formatEnvironmentDebug() {
    return DEBUG_ENV_KEYS.map((key) => `${key}:${formatEnvironmentDebugValue(key, process.env[key])}`).join(" ");
}
export function formatEnvironmentDebugValue(key, value) {
    if (value === undefined) {
        return "unset";
    }
