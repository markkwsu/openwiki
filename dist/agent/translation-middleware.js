...
import { OPENWIKI_TRANSLATION_PENDING_FIELD, readFrontmatterField, removeFrontmatterField, setFrontmatterField, } from "../okf/frontmatter.js";
/**
 ...
 */
export function resolveTranslationPlan(command, requestedLanguage, currentWikiLanguage) {
    if (command !== "update") {
        return undefined;
    }
    ...
    return {
        ...
        source,
        translateAll: requestedLanguage !== undefined &&
 ...
 */
export function createWikiTranslationMiddleware(backend, outputMode, model, plan, onWarning = (message) => {
    ...
    return createMiddleware({
        ...
        beforeAgent: async () => {
            await translateWiki(backend, outputMode, model, plan, onWarning, onStatus);
        },
    });
}
/**
 ...
 * @param onStatus - User-visible status sink.
 */
async function translateWiki(backend, outputMode, model, plan, onWarning, onStatus) {
    ...
    for (const filePath of await collectMarkdownFiles(backend, root)) {
        ...
        const pending = readFrontmatterField(content, OPENWIKI_TRANSLATION_PENDING_FIELD) !==
            undefined;
        ...
        try {
            ...
            const stampError = await markPending(backend, filePath, content, plan.target);
            if (stampError) {
    ...
    if (failures.length > 0) {
        onWarning(`OpenWiki could not translate ${failures.length} page(s) into ` +
 ...
 * @param plan - Resolved language transition.
 */
async function translatePage(backend, model, filePath, original, plan) {
    if (!original.trim())
        return;
    const translated = await translateMarkdown(model, original, plan.source, plan.target);
    if (!translated.trim()) {
        throw new Error("the model returned an empty translation");
    }
    const finalized = removeFrontmatterField(translated, OPENWIKI_TRANSLATION_PENDING_FIELD);
 ...
 * than swallow it.
 */
async function markPending(backend, filePath, content, target) {
    const stamped = setFrontmatterField(content, OPENWIKI_TRANSLATION_PENDING_FIELD, target);
    if (stamped === content)
        return undefined;
 ...
 */
async function translateMarkdown(model, content, from, to) {
 ...
 */
function buildTranslationPrompt(from, to) {
    return `You are a professional technical translator for a software documentation wiki.
Translate the Markdown document provided by the user into ${describeLanguage(to)}. It is expected to be in ${describeLanguage(from)}, but detect its actual language and translate any content that is not already in ${describeLanguage(to)}. If the document is already entirely in ${describeLanguage(to)}, return it unchanged.
 ...
 */
async function collectMarkdownFiles(backend, directoryPath) {
