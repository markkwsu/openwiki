...
import { lstat, mkdir, open, readdir, readFile, readlink, writeFile, } from "node:fs/promises";
import path from "node:path";
import { promisify } from "node:util";
import { OPEN_WIKI_DIR, PAGE_MANIFEST_PATH, UPDATE_METADATA_PATH, } from "../config/constants.js";
import { isExpectedSnapshotRaceError, isFileNotFoundError, } from "../platform/fs-errors.js";
import { getPrimaryLanguageSubtag, requireResolvedLanguage, } from "../platform/language.js";
import { readOpenWikiOnboardingConfig, readRepositoryWikiInstructions, } from "../setup/onboarding.js";
 ...
 */
export async function createRunContext(cwd, outputMode = "repository", language) {
 ...
 */
export async function getUpdateNoopStatus(cwd, openWikiIgnore = new OpenWikiIgnore([]), requestedLanguage) {
    ...
    if (resolvedRequestedLanguage !== undefined &&
    ...
    if (head !== lastUpdate.gitHead) {
        ...
        if (committedPaths.length === 0 ||
            committedPaths.some((changedPath) => !isOpenWikiPath(changedPath) && !openWikiIgnore.ignores(changedPath))) {
 ...
 */
export async function writeLastUpdateMetadata(command, cwd, modelId, outputMode = "repository", status = "complete", language, gitHeadOverride) {
    const metadataFile = getMetadataFilePath(cwd, outputMode);
    const gitHead = outputMode !== "repository"
        ? undefined
 ...
 */
export async function persistRunMetadataIfChanged(command, cwd, modelId, outputMode, snapshotBefore, status = "complete", language) {
    if (command === "chat" || snapshotBefore === null) {
        return false;
    }
    await writeLastUpdateMetadata(command, cwd, modelId, outputMode, status, language);
    return true;
}
/**
 ...
 */
export async function createRepositorySourceSnapshot(cwd, openWikiIgnore) {
    if (!path.isAbsolute(cwd)) {
        throw new Error("Repository source fingerprint requires an absolute root.");
    }
    const [head, trackedOutput, untrackedOutput, statusOutput] = await Promise.all([
    ...
    const trackedPaths = new Set(splitFingerprintNul(trackedOutput).map(assertFingerprintGitPath));
    ...
    const statusEntries = parseFingerprintStatus(statusOutput)
        .filter(({ path: candidate }) => isFingerprintSourcePath(candidate, openWikiIgnore))
        .sort((left, right) => compareFingerprintStrings(`${left.code}\u0000${left.path}`, `${right.code}\u0000${right.path}`));
    const hash = createHash("sha256");
    ...
    for (const sourcePath of visiblePaths) {
        await updateFingerprintSourceEntry(hash, cwd, sourcePath, trackedPaths.has(sourcePath));
    }
 ...
 */
export async function createRepositorySourceFingerprint(cwd, openWikiIgnore) {
 ...
 */
async function readFingerprintHead(cwd) {
    try {
        const head = (await runFingerprintGit(cwd, ["rev-parse", "--verify", "HEAD"])).trimEnd();
        if (!head)
            throw new Error("Git returned an empty HEAD.");
        return head;
    }
    catch (headError) {
        try {
            const symbolicHead = (await runFingerprintGit(cwd, ["symbolic-ref", "-q", "HEAD"])).trimEnd();
            if (symbolicHead)
                return `unborn:${symbolicHead}`;
        }
        catch {
 ...
 */
async function runFingerprintGit(cwd, args) {
    try {
        ...
        throw new Error(`Git failed while creating the repository source fingerprint: git ${args.join(" ")}`, { cause: error });
    }
}
/**
 * Decodes a complete NUL-terminated Git record stream without filename loss.
 */
function splitFingerprintNul(output) {
    if (output.length === 0)
        return [];
    if (!output.endsWith("\u0000")) {
 ...
 */
function assertFingerprintGitPath(value) {
 ...
 */
function isFingerprintSourcePath(candidate, openWikiIgnore) {
    if (candidate === OPENWIKI_IGNORE_FILE)
        return true;
 ...
 */
async function fingerprintEntryExists(filePath) {
    try {
 ...
 */
async function updateFingerprintSourceEntry(hash, cwd, sourcePath, tracked) {
    ...
    if (!absolutePath.startsWith(`${absoluteRoot}${path.sep}`)) {
        throw new Error(`Source fingerprint path escaped the repository: ${sourcePath}`);
    }
    ...
    if (stats.isFile()) {
        const file = await readFingerprintRegularFile(absolutePath, sourcePath, stats);
    ...
    updateFingerprintField(hash, "executable", (stats.mode & 73n) !== 0n ? "yes" : "no");
    if (stats.isSymbolicLink()) {
        updateFingerprintField(hash, "kind", "symlink");
        updateFingerprintField(hash, "target", await readlink(absolutePath, { encoding: "buffer" }));
        return;
    }
 ...
 */
async function readFingerprintRegularFile(absolutePath, sourcePath, inspectedStats) {
    let fileHandle;
    try {
    ...
    try {
        ...
        if (!isSameFingerprintRegularFile(inspectedStats, openedStats)) {
            throw new Error(`Source path changed while fingerprinting ${sourcePath}.`);
        }
        return {
            bytes: await fileHandle.readFile(),
            executable: (openedStats.mode & 73n) !== 0n,
        };
    }
    finally {
...
function isSameFingerprintRegularFile(inspectedStats, openedStats) {
    if (!openedStats.isFile()) {
        return false;
    }
    if (process.platform !== "win32") {
        return (openedStats.dev === inspectedStats.dev &&
    ...
    return (openedStats.size === inspectedStats.size &&
 ...
 */
function getFingerprintFileOpenFlags() {
 ...
 */
function updateFingerprintField(hash, label, value) {
 ...
 */
function compareFingerprintStrings(left, right) {
    return left < right ? -1 : left > right ? 1 : 0;
}
/**
 * Reads prior run metadata if it exists and is structurally valid.
 */
async function readLastUpdate(cwd, outputMode) {
    const metadataFile = getMetadataFilePath(cwd, outputMode);
    try {
        ...
        if (typeof parsedMetadata.updatedAt === "string" &&
            ...
            return {
                ...
                status: parsedMetadata.status === "interrupted" ? "interrupted" : "complete",
 ...
 */
function isIgnoredSnapshotPath(relativePath) {
    return (relativePath === path.basename(UPDATE_METADATA_PATH) ||
...
function isUpdateMetadataStatusLine(line) {
    ...
    return [PAGE_MANIFEST_PATH, UPDATE_METADATA_PATH].some((metadataPath) => normalizedPath === metadataPath ||
 ...
 */
export async function getRepositoryChangedPaths(cwd, openWikiIgnore, baseGitHead) {
    const paths = new Set();
    if (baseGitHead) {
 ...
 */
function lineReferencesIgnoredPath(line, openWikiIgnore) {
    return extractGitPaths(line).some((changedPath) => openWikiIgnore.ignores(changedPath));
}
/**
 ...
 */
function splitGitPaths(pathsText) {
