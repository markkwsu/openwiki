import { chmod, cp, lstat, mkdir, mkdtemp, readdir, rename, rm, stat, } from "node:fs/promises";
import { fileURLToPath } from "node:url";
import path from "node:path";
import { ensureOpenWikiHome, openWikiSkillsDir, } from "../config/openwiki-home.js";
...
export async function replaceSkillDirectories(sourceDir, targetDir) {
    const skills = (await readdir(sourceDir, { withFileTypes: true })).filter((entry) => entry.isDirectory());
    await mkdir(targetDir, { recursive: true });
    await Promise.all(skills.map(({ name }) => replaceSkillDirectory(sourceDir, targetDir, name)));
}
/**
 ...
 */
async function replaceSkillDirectory(sourceDir, targetDir, name) {
    ...
    try {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
            try {
                ...
                // retry the latter.
                const installed = await stat(target).then((stats) => stats.isDirectory(), () => false);
                if (installed) {
                    installError = undefined;
                    break;
                }
 ...
 */
async function grantOwnerWrite(entry) {
    ...
    if (stats.isDirectory()) {
        ...
        await Promise.all(children.map((child) => grantOwnerWrite(path.join(entry, child))));
    }
}
