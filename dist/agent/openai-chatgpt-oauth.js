...
import { OPENAI_CHATGPT_ACCESS_TOKEN_ENV_KEY, OPENAI_CHATGPT_ACCOUNT_ID_ENV_KEY, OPENAI_CHATGPT_EMAIL_ENV_KEY, OPENAI_CHATGPT_EXPIRES_AT_ENV_KEY, OPENAI_CHATGPT_PLAN_ENV_KEY, OPENAI_CHATGPT_REFRESH_TOKEN_ENV_KEY, } from "../config/constants.js";
/**
 ...
 */
export function createCodexFetch(modelId, fetchImpl = globalThis.fetch) {
    return async (input, init) => {
        const isCodexRequest = isCodexResponsesRequest(input);
        const useLunaProtocol = modelId === CODEX_LUNA_MODEL_ID && isCodexRequest;
        const omitPromptCacheRetention = (modelId === "gpt-5.6" || modelId.startsWith("gpt-5.6-")) &&
            isCodexRequest;
 ...
 */
export function codexTokensToEnv(tokens) {
    return {
 ...
 */
export function readCodexTokensFromEnv(env = process.env) {
...
export function formatChatGptAccount(email, planType) {
    const plan = planType
...
export function formatChatGptAccountFromEnv(env = process.env) {
    return formatChatGptAccount(env[OPENAI_CHATGPT_EMAIL_ENV_KEY] ?? null, env[OPENAI_CHATGPT_PLAN_ENV_KEY] ?? null);
}
...
function isCodexResponsesRequest(input) {
...
async function exchangeToken(body) {
    ...
    if (!res.ok) {
        throw new Error(`ChatGPT token request failed (${res.status}). Try signing in again.`);
    }
    const json = (await res.json());
    const missing = ["access_token", "refresh_token", "expires_in"].filter((field) => json[field] === undefined || json[field] === null);
    if (missing.length > 0) {
        throw new Error(`ChatGPT token response missing required fields: ${missing.join(", ")}.`);
    }
 ...
 */
export function parseManualCallbackInput(input) {
...
export async function loginWithChatGPT(openUrl, onReady) {
    ...
    const code = await new Promise((resolve, reject) => {
        ...
        const server = http.createServer((req, res) => {
            ...
            res
                ...
                .end("<html><body>OpenWiki login complete. You can close this tab.</body></html>");
            finish(authCode);
        });
        ...
        server.listen(CALLBACK_PORT, "localhost", () => {
            ...
            onReady?.({
                submitManual(rawInput) {
                    const { code: manualCode, state: manualState } = parseManualCallbackInput(rawInput);
                    if (!manualCode) {
 ...
 */
export async function refreshChatGptTokens(refreshToken) {
    return exchangeToken(new URLSearchParams({
 ...
 */
export function isChatGptTokenExpired(expiresAtMs, now = Date.now(), thresholdMs = CHATGPT_TOKEN_REFRESH_THRESHOLD_MS) {
