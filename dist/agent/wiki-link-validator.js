 ...
 */
const BROKEN_LINK_STAMP_PATTERN = /^\s*<!--\s*openwiki:\s*broken internal link\b.*?-->\s*$/u;
/**
 ...
 */
export async function validateWikiInternalLinks(backend, outputMode) {
    ...
    for (const sourcePath of await collectMarkdownFiles(backend, wikiRoot)) {
        ...
        for (const { href, line } of extractMarkdownLinks(cleaned)) {
            ...
            const issue = await validateLink(backend, sourcePath, href, line, headingAnchors);
            if (issue) {
                issues.push(issue);
            }
        }
 ...
 */
export function formatWikiLinkIssues(issues) {
    const lines = issues.map((issue) => `${issue.sourcePath}:${issue.line} [${issue.href}] ${issue.message}`);
 ...
 */
export function formatBrokenLinkStamp(href, message) {
    return (`<!-- openwiki: broken internal link [${href}] ${message}. ` +
 ...
 */
export function stripBrokenLinkStamps(content) {
    return content
        .split(/\r?\n/u)
 ...
 */
export function stampBrokenLinks(content, issues) {
    if (issues.length === 0) {
        return content;
    }
    ...
    for (const lineNumber of [...byLine.keys()].sort((a, b) => b - a)) {
        const stamps = (byLine.get(lineNumber) ?? []).map((issue) => formatBrokenLinkStamp(issue.href, issue.message));
 ...
 */
async function validateLink(backend, sourcePath, rawHref, line, sourceAnchors) {
 ...
 */
function parseLinkDestination(rawHref) {
 ...
 */
function resolveRepoLinkPath(sourcePath, linkPath) {
    const candidate = path.posix.normalize(linkPath.startsWith("/")
        ? linkPath
 ...
 */
async function pathExists(backend, targetPath, isDirectory) {
    try {
        if (isDirectory) {
