...
export class OpenWikiIgnoreRule {
    ...
    static compile(pattern) {
        ...
        return new OpenWikiIgnoreRule(directoryOnly, OpenWikiIgnoreRule.createMatcher(normalizedPattern, anchored), negated);
    }
    /**
     ...
     */
    static createMatcher(pattern, anchored) {
     ...
     */
    static globToRegexSource(pattern) {
        let source = "";
     ...
     */
    negated;
    constructor(directoryOnly, matcher, negated) {
...
export class OpenWikiIgnore {
    ...
    static parse(contents) {
        return new OpenWikiIgnore(contents
            .split(/\r?\n/u)
            .map(parseIgnoreLine)
    ...
    static async load(cwd) {
        try {
            const contents = await readFile(path.join(cwd, OPENWIKI_IGNORE_FILE), "utf8");
