import { CLAIMS_RECONCILIATION_GUIDANCE, CLAIMS_SUBSTANCE_GUIDANCE, } from "../claims/guidance.js";
/**
 ...
 */
export function createRepositoryPlannerPrompt(view, planningContext) {
    const updateContext = view.mode === "update"
        ? `
 ...
 */
export function createRepositoryPagePrompt(job, allPages, language) {
...
function formatIssues(issues) {
    return issues.length > 0
        ? issues
            .map((issue) => `- ${issue.page}: ${issue.claimId} (${issue.kind}) -> ${issue.resources.join(", ")}`)
            .join("\n")
        : "- (none)";
}
/**
