...
const OPENAI_MAAS_MODEL_PATTERN = /(^|\/)(ai21|codellama|codestral|deepseek|jamba|llama|meta|mistral|qwen)/u;
/**
 ...
 */
export function resolveVertexSurface(modelId) {
 ...
 * unchanged.
 */
export function stripPublisherPath(modelId) {
 ...
 */
export function toVertexPublisherModel(modelId) {
 ...
 * is race-free.
 */
export function withAnthropicAuthEnvNeutralized(construct) {
    const saved = ANTHROPIC_NATIVE_AUTH_ENV_KEYS.map((key) => [key, process.env[key]]);
 ...
 */
export function createVertexAuthFetch(baseFetch = fetch) {
    const auth = getGoogleAuth();
    return async (input, init) => {
        const token = await auth.getAccessToken();
        if (!token) {
            throw new Error("Failed to obtain a Google Cloud access token for Vertex AI. Check Application Default Credentials (gcloud auth application-default login).");
        }
