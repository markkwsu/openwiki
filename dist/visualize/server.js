 ...
 */
export async function runVisualizeServer(options) {
    ...
    const rebuild = async (reason) => {
        try {
            ...
            process.stdout.write(`  ↻ ${reason}: ${graph.nodes.length} pages, ${graph.edges.length} links\n`);
            broadcastReload();
        }
        catch (error) {
 ...
 */
async function assertWikiDir(wikiRoot) {
    try {
        ...
        throw new Error(`Wiki directory not found: ${wikiRoot}. Run \`openwiki --init\` first, or pass a path.`);
    }
}
/**
 * Try the preferred port, incrementing on EADDRINUSE.
 */
function listen(server, port, attemptsLeft, onReady) {
    const onError = (err) => {
 ...
 */
function startWatch(wikiRoot, rebuild) {
    let timer;
    try {
        watch(wikiRoot, { recursive: true }, () => {
            clearTimeout(timer);
            timer = setTimeout(() => void rebuild("change detected"), WATCH_DEBOUNCE_MS);
        });
    }
    catch {
 ...
 */
function printBanner(wikiRoot, url) {
    process.stdout.write(`\n  OpenWiki visualizer\n`);
    process.stdout.write(`  wiki:  ${wikiRoot}\n`);
    process.stdout.write(`  open:  \x1b[36m${url}\x1b[0m\n`);
    process.stdout.write(`  live:  editing pages under the wiki refreshes the browser\n\n`);
