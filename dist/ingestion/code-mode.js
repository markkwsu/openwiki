...
import { getProviderAuthMethod, getProviderConfig, OPENAI_COMPATIBLE_STREAMING_ENV_KEY, OPENWIKI_MODEL_ID_ENV_KEY, OPENWIKI_VERSION, resolveConfiguredProvider, resolveOpenAiCompatibleStreaming, } from "../config/constants.js";
 ...
 */
export async function ensureCodeModeRepoSetup(cwd, options = {}) {
    if (options.createWorkflow) {
        await ensureCodeModeWorkflow(cwd, options.cronExpression ?? DEFAULT_CODE_MODE_CRON, options.env ?? process.env);
    }
 ...
 */
export async function runCodeModeConnectors(repoRoot, baseMessage, onEvent) {
 ...
 */
function windowHoursSince(since) {
 ...
 */
async function readLastUpdatedAt(repoRoot) {
    try {
        const text = await readFile(path.join(repoRoot, UPDATE_METADATA_PATH), "utf8");
...
async function writeCodeModeAgentSnippets(cwd) {
    ...
    const updates = await Promise.all(CODE_MODE_AGENT_FILES.map((fileName) => prepareCodeModeAgentSnippet(path.join(cwd, fileName), snippetByFile[fileName] ?? agentsSnippet)));
    await Promise.all(updates.map(({ agentsPath, nextContent }) => nextContent === undefined
        ? Promise.resolve()
        : writeFile(agentsPath, nextContent, "utf8")));
}
async function prepareCodeModeAgentSnippet(agentsPath, snippet) {
    let currentContent = "";
    try {
    ...
    if (path.basename(agentsPath) === "CLAUDE.md" &&
    ...
    const hasOneOrderedPair = startIndex !== -1 &&
        endIndex > startIndex &&
    ...
    if (!hasOneOrderedPair) {
        throw new Error(`Cannot update ${path.basename(agentsPath)} because its OpenWiki managed markers are malformed or duplicated. Expected either no markers or exactly one ${OPENWIKI_AGENTS_SNIPPET_START} marker followed by one ${OPENWIKI_AGENTS_SNIPPET_END} marker. Repair or remove the markers and retry; the file was left unchanged.`);
    }
    return {
        agentsPath,
 ...
 */
function createWorkflowProviderEnv(env) {
    ...
    if (getProviderAuthMethod(provider) === "oauth") {
        ...
        lines.push(`# ${config.label} authenticates through a browser login, which has no`, "# unattended equivalent. Supply CI credentials for it yourself.");
    }
    else if (config.apiKeyEnvKey !== undefined) {
        lines.push(`${config.apiKeyEnvKey}: \${{ secrets.${config.apiKeyEnvKey} }}`);
        if (config.secretKeyEnvKey !== undefined) {
            lines.push(`${config.secretKeyEnvKey}: \${{ secrets.${config.secretKeyEnvKey} }}`);
        }
    }
    ...
    const modelId = env[OPENWIKI_MODEL_ID_ENV_KEY]?.trim() || config.modelOptions[0]?.id;
...
function createCodeModeWorkflow(cronExpression, env) {
...
function createCodeModeAgentsSnippet() {
 ...
 * it reads at startup.
 */
function createCodeModeClaudeSnippet() {
    return `${OPENWIKI_AGENTS_SNIPPET_START}
