import { createConnectorRegistry, isConnectorId, } from "../connectors/registry.js";
import { loadOpenWikiEnv } from "../config/env.js";
import { readOpenWikiOnboardingConfig, } from "../setup/onboarding.js";
import { ensureOpenWikiHome, getConnectorConfigPath, openWikiHomeDisplayPath, openWikiLocalWikiDir, openWikiLocalWikiDisplayPath, } from "../config/openwiki-home.js";
import { createOpenWikiThreadId, runOpenWikiAgent } from "../agent/index.js";
import { withRunTelemetry, } from "../telemetry/index.js";
const INGESTION_WINDOW_HOURS = 24;
export async function runOpenWikiIngestion(_cwd = process.cwd(), options) {
    void _cwd;
    ...
    if (options.target !== "all" && sourceInstances.length === 0) {
        throw new Error(`No configured ingestion source matched ${formatTarget(options.target)}.`);
    }
    for (const sourceConfig of sourceInstances) {
        const connector = registry[sourceConfig.connectorId];
        results.push(await runSourceIngestion({
            config,
            connector,
...
export function parseIngestionTarget(value) {
    if (value === "all") {
        return "all";
    }
...
async function runSourceIngestion({ config, connector, cwd, emit, modelId, sourceConfig, }) {
    ...
    try {
        ...
        if (deterministicPull &&
            ...
            rawFiles.length === 0) {
            emitText(emit, `${connector.displayName} deterministic pull failed: ${deterministicPull.message}\n`);
            return {
        ...
        const agentResult = await withRunTelemetry("update", runOptions, telemetryContext, () => runOpenWikiAgent("update", cwd, runOptions, telemetryContext));
        return {
            agentResult,
            connectorId: connector.id,
            deterministicPull,
...
function resolveIngestionSourceInstances(target, config, { scheduledOnly }) {
...
function isSafeSourceInstanceId(value) {
    return /^[A-Za-z0-9][A-Za-z0-9._-]{0,119}$/u.test(value);
}
function formatTarget(target) {
    return typeof target === "object" ? target.id : target;
}
function getSourceDisplayName(connector, sourceConfig) {
    return sourceConfig.name ?? connector.displayName;
}
function isDeterministicConnector(connector) {
    return !connector.supportsAgenticDiscovery;
}
function createSourceUpdateMessage({ config, connector, deterministicPull, rawFiles, sourceConfig, }) {
...
function createSourceSynthesisPolicy(connector) {
    return `
...
export function createConnectorSynthesisGuidance(connector) {
    switch (connector.id) {
        case "google":
            return `
...
function emitDeterministicPullSummary(emit, deterministicPull) {
    if (!deterministicPull) {
        return;
    }
    emitText(emit, `${deterministicPull.message} Raw files: ${deterministicPull.rawFiles.length > 0
...
function formatRawFileList(rawFiles) {
