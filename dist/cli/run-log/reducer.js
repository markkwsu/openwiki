 ...
 */
export function appendRunLogEvent(log, event, nextLogId) {
    if (event.type === "repository_progress") {
        ...
        return existing
            ? log.map((item) => item.type === "repository_progress" ? progress : item)
            : [progress, ...log];
    }
    if (event.type === "text") {
    ...
    const withoutOldestDebug = debugItems.length >= MAX_DEBUG_ITEMS
...
function appendToolStartLogItem(log, event, nextLogId) {
    ...
    const readCount = (previousSummary?.readCount ?? 0) + (event.name === "read_file" ? 1 : 0);
    const searchCount = (previousSummary?.searchCount ?? 0) +
        (["glob", "grep", "ls"].includes(event.name) ? 1 : 0);
    const taskCount = (previousSummary?.taskCount ?? 0) +
        (event.name === "task"
            ? countToolTargets(event.input, ["tasks", "subagents", "agents", "items"])
            : 0);
    const writeCount = (previousSummary?.writeCount ?? 0) +
    ...
    let nextLog = summaryIndex === -1
        ? [summary, ...withoutNarration]
        : withoutNarration.map((item, index) => index === summaryIndex ? summary : item);
...
function completeToolLogItem(log, event) {
    ...
    const writtenPaths = event.status === "finished"
        ? log.flatMap((item) => item.type === "activity" &&
            ...
            : [])
        : [];
    const exploredPaths = event.status === "finished"
        ? log.flatMap((item) => item.type === "activity" &&
            ...
            : [])
        : [];
    const touchedActivityIds = new Set(log
        .filter((item) => item.type === "activity" &&
        getActiveToolCallIds(item).includes(event.id))
        .map((item) => item.id));
    const completed = log.map((item, index) => {
        ...
        return {
            ...item,
            activeToolCallIds,
            activityStatus: activeToolCallIds.length > 0
                ? "active"
                : event.status === "error"
                    ? "error"
                    : "recent",
 ...
 */
function completeToolGroupItem(item, event, writtenPaths = [], exploredPaths = []) {
    ...
    return {
        ...
        status: activeToolCallIds.length > 0
            ? "running"
            : errorCount > 0
                ? "error"
                : "done",
...
function findLastToolLogItemIndex(log, toolCallId) {
    return log.findIndex((item) => item.type === "tool" && getActiveToolCallIds(item).includes(toolCallId));
}
/**
...
function activatePath(log, activity, toolCallId, nextLogId) {
    const matching = log.find((item) => item.type === "activity" &&
...
function boundActivityLog(log) {
    const nonActivities = log.filter((item) => item.type !== "activity");
    const activeActivities = log.filter((item) => item.type === "activity" && item.activityStatus === "active");
    const recentActivities = log
        .filter((item) => item.type === "activity" && item.activityStatus !== "active")
        .slice(-MAX_RECENT_ACTIVITIES);
    return [...nonActivities, ...activeActivities, ...recentActivities];
}
function removeItemById(log, id) {
