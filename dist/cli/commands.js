...
import { parseIngestionTarget, } from "../ingestion/ingestion.js";
...
export function parseCommand(argv) {
    ...
    if (argv[0] === "auth") {
        ...
        const unknownOption = optionArgs.find((arg) => acceptsForce ? arg !== "--force" : true);
    ...
    if (argv[0] === "ingest") {
        ...
        if (!target) {
            return {
                kind: "error",
                exitCode: 1,
                message: "Usage: openwiki ingest <source|source-instance|all> [--scheduled] [--print] [--modelId <id>]",
            };
        }
        let modelId = null;
        let print = false;
    ...
    if (argv[0] === "cron") {
        ...
        }
        {
            return {
                kind: "error",
                exitCode: 1,
                message: "Usage: openwiki cron list | pause all | resume all | delete all",
            };
        }
    }
 ...
 */
function parseIntegrationsCommand(argv) {
    const action = argv[0];
        ...
        if (arg === "--host" || arg.startsWith("--host=")) {
            ...
            if (!isValidHostId(value)) {
                return {
                    kind: "error",
                    exitCode: 1,
                    message: "--host must contain 1-64 lowercase letters, digits, or hyphens.",
                };
            }
            host = value;
            sawHost = true;
 ...
 */
function integrationUsageError() {
    return {
        kind: "error",
        exitCode: 1,
        message: "Usage: openwiki integrations list [--project [path]] | " +
 ...
 */
function formatSupportedHostTargets(separator = ", ") {
...
function parseRunCommand(argv, initialMode, initialModeSource) {
    ...
    return {
        ...
        command,
        dryRun,
        language: resolvedLanguage.kind === "resolved" ? resolvedLanguage.language : null,
        mode,
        modeSource,
        modelId,
        print,
 ...
 */
export function commandEmitsTelemetry(command) {
    return (command.kind === "run" &&
        !command.dryRun &&
 ...
 */
export function commandLoadsEnvironment(command) {
    return ((command.kind === "run" && !command.dryRun) ||
        command.kind === "auth" ||
        command.kind === "cron" ||
        command.kind === "ingest" ||
        command.kind === "ngrok");
...
export const helpContent = {
    ...
    commands: [
        ...
        {
            label: "openwiki",
            description: "Open the interactive OpenWiki code chat for the current repository.",
        },
        {
        ...
        },
        {
            ...
            description: "Serve a live graph and reader, or export a static graph for web hosting (defaults to ./openwiki).",
        },
        {
            label: "openwiki integrations list [--project [path]]",
            description: "Show user-level OpenWiki installation status, or project status with --project.",
        },
        {
            label: "openwiki integrations install <host> [--project [path]]",
            description: "Install the OpenWiki skill and MCP config globally, or into one project with --project.",
        },
        {
            label: "openwiki integrations uninstall <host> [--project [path]]",
            description: "Safely remove a global integration, or a project integration with --project.",
        },
    ],
    options: [
        {
            label: "--init",
            description: "Generate repository documentation from scratch, replacing an existing generated wiki while preserving openwiki/INSTRUCTIONS.md. Defaults to code mode; use personal to initialize the local personal brain.",
        },
        {
            label: "--update",
            description: "Update existing OpenWiki documentation. Defaults to code mode; use personal to update the local personal brain.",
        },
        {
        ...
        },
        {
            ...
            description: "Generate wiki documentation in the given BCP-47 locale, for example ko, zh-CN, or pt-BR.",
        },
        {
            label: "-p, --print",
        ...
        {
            label: "--debug",
            description: "Show full credential and error diagnostics when a run fails.",
        },
        {
            label: "--modelId <id>",
        ...
        {
            label: "--scheduled",
            description: "For ingest only: run scheduled-only ingestion for scheduler-managed runs.",
        },
        {
            label: "--telemetry-file <path>",
            description: "Write the exact anonymous telemetry payload to a local JSON file.",
        },
        {
            label: "--port <port>",
            description: "For visualize: port to serve on (default 4321; increments on conflict).",
        },
        {
            label: "--no-open",
        ...
        {
            label: "--export <dir>",
            description: "For visualize: write a static visualizer directory instead of starting the local server.",
        },
    ],
    developmentOptions: [
        {
