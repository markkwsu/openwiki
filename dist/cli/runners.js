...
import { deleteConnectorSchedules, getSavedPowerScheduleStatus, listConnectorSchedules, pauseConnectorSchedules, resumeConnectorSchedules, } from "../scheduling/schedules.js";
import { readOpenWikiOnboardingConfig, saveOpenWikiOnboardingConfig, } from "../setup/onboarding.js";
...
import { formatPowerScheduleStatus, formatScheduleHeader, formatScheduleMutationResult, formatScheduleStatus, } from "./schedule-format.js";
export async function runNgrokCommand(command) {
    try {
 ...
 */
export async function runVisualizeCommand(command) {
    ...
    try {
        if (command.exportDir) {
            ...
            process.stdout.write(`Exported static visualizer to ${result.outputDir} (${result.graph.nodes.length} pages, ${result.graph.edges.length} links).\n`);
            return;
        }
...
export async function runCronCommand(command) {
    try {
...
export async function runIngestCommand(command) {
    try {
        ...
        const hadError = result.results.some((sourceResult) => sourceResult.status === "error");
...
export async function runAuthCommand(command) {
    try {
        if (command.action === "list") {
 ...
 */
export async function runPrintCommand(command) {
    try {
        const output = [];
        ...
        await withRunTelemetry(command.command, runOptions, telemetryContext, async () => {
            ...
            // interactive.
            const userMessage = command.mode === "code" && command.command !== "chat"
                ? await runCodeModeConnectors(runtimeCwd, command.userMessage ?? undefined, handlePrintEvent)
                : command.userMessage;
            await runOpenWikiAgent(command.command, runtimeCwd, { ...runOptions, userMessage }, telemetryContext);
        });
 ...
 */
export function writePrintAuthFix(error, message) {
...
export function writePrintErrorDiagnostics(error) {
