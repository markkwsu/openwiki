import { ANTHROPIC_API_KEY_ENV_KEY, AWS_ACCESS_KEY_ID_ENV_KEY, AWS_BEARER_TOKEN_BEDROCK_ENV_KEY, AWS_SECRET_ACCESS_KEY_ENV_KEY, AWS_SESSION_TOKEN_ENV_KEY, AWS_WEB_IDENTITY_TOKEN_FILE_ENV_KEY, BASETEN_API_KEY_ENV_KEY, BEDROCK_AWS_ACCESS_KEY_ID_ENV_KEY, BEDROCK_AWS_SECRET_ACCESS_KEY_ENV_KEY, BEDROCK_AWS_SESSION_TOKEN_ENV_KEY, COPILOT_API_KEY_ENV_KEY, FIREWORKS_API_KEY_ENV_KEY, GEMINI_API_KEY_ENV_KEY, NEBIUS_API_KEY_ENV_KEY, NVIDIA_API_KEY_ENV_KEY, OPENAI_API_KEY_ENV_KEY, OPENAI_COMPATIBLE_API_KEY_ENV_KEY, OPENROUTER_API_KEY_ENV_KEY, } from "../config/constants.js";
/**
 ...
 */
export const SECRET_KEY_PATTERN_SOURCE = "api[-_]?key|authorization|bearer|token|secret|password|user_id|cookie";
 ...
 */
export function isOpenRouterServerError(error, message) {
    if (isRecord(error)) {
 ...
 */
export function isAuthError(error, message) {
    ...
    return /\b40[13]\b|incorrect api key|invalid api key|unauthorized|forbidden|authentication|permission denied|not authorized|could not load credentials from any providers|invalid identity token|expired token|web identity token.+(?:expired|invalid|not valid)|security token.+invalid/iu.test(message);
}
