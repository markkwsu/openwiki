 ...
 */
export function stripTerminalControlSequences(input) {
    let output = "";
...
function skipEscapeSequence(input, index) {
...
function skipC1Sequence(input, index) {
    const code = input.charCodeAt(index);
    if (code === 0x9b) {
        return skipCsiSequence(input, index + 1);
    }
...
function skipCsiSequence(input, index) {
...
function skipStringSequence(input, index) {
    while (index < input.length) {
        const code = input.charCodeAt(index);
