...
```sh
pnpm integrations:dev <codex|claude|opencode|cursor|bob>
```
...
agent after installation. Codex, Claude Code, OpenCode, Cursor, and IBM Bob
install at user scope. Later source changes only require `pnpm build` unless the
bundled skill itself changes. Rerun `integrations:dev` to refresh the skill or
after switching Node installations.
...
supported platform), Cursor under `~/.cursor`, and IBM Bob under
`~/.bob/settings`.
